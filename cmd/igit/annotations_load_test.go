...
func TestParseArgs_AnnotationsFlag(t *testing.T) {
	...
	assert.Equal(t, "/tmp/notes.md", opts.Review.Annotations)
}

func TestParseArgs_AnnotationsDefaultEmpty(t *testing.T) {
	opts, err := parseArgs(noConfigArgs(t))
	require.NoError(t, err)
	assert.Empty(t, opts.Review.Annotations)
}
...
func TestPreloadAnnotations_ChangedFilesError(t *testing.T) {
	...
	store := annot.NewStore()
	r := &mocks.DiffSourceMock{
		ChangedFilesFunc: func(string, bool) ([]git.FileEntry, error) { return nil, errors.New("boom") },
	}
	err := preloadAnnotations(path, store, r, "", false, nil, nil, "", &bytes.Buffer{})
	require.Error(t, err)
...
func TestPreloadAnnotations_DropsOrphans(t *testing.T) {
	...
	r := &mocks.DiffSourceMock{
		...
		FileDiffFunc: func(req git.FileDiffRequest) ([]git.DiffLine, error) {
			...
			return []git.DiffLine{
				{OldNum: 0, NewNum: 5, Content: "added", ChangeType: git.ChangeAdd},
				{OldNum: 4, NewNum: 0, Content: "removed", ChangeType: git.ChangeRemove},
				{OldNum: 6, NewNum: 6, Content: "ctx", ChangeType: git.ChangeContext},
			}, nil
		},
	}
	warn := &bytes.Buffer{}
...
func TestPreloadAnnotations_RemovedLineMatchesOldNum(t *testing.T) {
	...
	r := &mocks.DiffSourceMock{
		...
		FileDiffFunc: func(git.FileDiffRequest) ([]git.DiffLine, error) {
			return []git.DiffLine{
				{OldNum: 4, NewNum: 0, Content: "gone", ChangeType: git.ChangeRemove},
			}, nil
		},
	}
...
func TestPreloadAnnotations_ContextMatchesNewNum(t *testing.T) {
	...
	r := &mocks.DiffSourceMock{
		...
		FileDiffFunc: func(git.FileDiffRequest) ([]git.DiffLine, error) {
			return []git.DiffLine{
				{OldNum: 7, NewNum: 7, Content: "ctx", ChangeType: git.ChangeContext},
			}, nil
		},
	}
	require.NoError(t, preloadAnnotations(path, store, r, "", false, nil, nil, "", &bytes.Buffer{}))
...
func TestPreloadAnnotations_DuplicateLastWriteWins(t *testing.T) {
	...
	r := &mocks.DiffSourceMock{
		...
		FileDiffFunc: func(git.FileDiffRequest) ([]git.DiffLine, error) {
			return []git.DiffLine{{NewNum: 5, ChangeType: git.ChangeAdd}}, nil
		},
	}
	require.NoError(t, preloadAnnotations(path, store, r, "", false, nil, nil, "", &bytes.Buffer{}))
	assert.Equal(t, 1, store.Count())
...
func TestPreloadAnnotations_StagedOnlyAddedFile(t *testing.T) {
	...
	store := annot.NewStore()
	r := &mocks.DiffSourceMock{
		ChangedFilesFunc: func(_ string, staged bool) ([]git.FileEntry, error) {
			if staged {
				return []git.FileEntry{{Path: "new.go", Status: git.FileAdded}}, nil
			}
			return nil, nil
		},
		FileDiffFunc: func(req git.FileDiffRequest) ([]git.DiffLine, error) {
			...
			return []git.DiffLine{
				{NewNum: 1, Content: "first", ChangeType: git.ChangeAdd},
				{NewNum: 2, Content: "second", ChangeType: git.ChangeAdd},
			}, nil
		},
	}
	warn := &bytes.Buffer{}
...
func TestPreloadAnnotations_UntrackedFile(t *testing.T) {
	...
	// source.ChangedFiles. Annotations saved against them must round-trip:
...
func TestPreloadAnnotations_RenamedFileUsesRenameAwareDiff(t *testing.T) {
	...
	r := &mocks.DiffSourceMock{
		ChangedFilesFunc: func(string, bool) ([]git.FileEntry, error) {
			return []git.FileEntry{{Path: "new.go", OldPath: "old.go", Status: git.FileRenamed}}, nil
		},
		FileDiffFunc: func(req git.FileDiffRequest) ([]git.DiffLine, error) {
			gotReq = req
			return []git.DiffLine{
				{OldNum: 1, NewNum: 1, Content: "first", ChangeType: git.ChangeContext},
				{OldNum: 0, NewNum: 2, Content: "TWO", ChangeType: git.ChangeAdd},
			}, nil
		},
	}
	warn := &bytes.Buffer{}
...
func TestPreloadAnnotations_UntrackedRenameUsesRenameAwareDiff(t *testing.T) {
	...
	r := &mocks.DiffSourceMock{
		ChangedFilesFunc: func(string, bool) ([]git.FileEntry, error) {
			return []git.FileEntry{{Path: "old.go", Status: git.FileDeleted}}, nil
		},
		FileDiffFunc: func(req git.FileDiffRequest) ([]git.DiffLine, error) {
			gotReq = req
			return []git.DiffLine{
				{OldNum: 1, NewNum: 1, Content: "one", ChangeType: git.ChangeContext},
				{OldNum: 2, NewNum: 0, Content: "two", ChangeType: git.ChangeRemove},
				{OldNum: 0, NewNum: 2, Content: "TWO", ChangeType: git.ChangeAdd},
			}, nil
		},
	}
	untracked := func() ([]string, error) { return []string{"new.go"}, nil }
	untrackedRenames := func([]string) ([]git.FileEntry, error) {
		return []git.FileEntry{{Path: "new.go", OldPath: "old.go", Status: git.FileRenamed}}, nil
	}
	warn := &bytes.Buffer{}
...
func TestPreloadAnnotations_SanitizesCommentText(t *testing.T) {
	...
	// before reaching Store.Add - the TUI source wraps comment text in
