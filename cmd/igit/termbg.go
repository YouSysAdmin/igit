...
// termenv sends no OSC query under tmux or screen, so inside tmux three paths
// are tried in order: the current client's theme (tmux display-message), a
// server-wide client scan (tmux list-clients, for a nested client that has no
// theme of its own) and termenv's OSC 11 query with TERM masked. When all
// fail, termenv falls back to COLORFGBG and then to its dark default.
