...
const (
	autoThemeName         = "auto"
	defaultAutoThemeDark  = "default"
	defaultAutoThemeLight = "basic"
)

// defaultThemesDir returns ~/.config/igit/themes.
func defaultThemesDir() string {
	home, err := os.UserHomeDir()
	if err != nil {
		return ""
	}
	return filepath.Join(home, ".config", "igit", "themes")
}
...
// compile-time check: *themeCatalog satisfies tui.ThemeCatalog.
var _ tui.ThemeCatalog = (*themeCatalog)(nil)

// themeCatalog adapts theme.Catalog + config-patching persistence into the tui.ThemeCatalog interface.
// it composes theme discovery (from internal/theme) with selected-theme persistence (config file patching).
...
func (tc *themeCatalog) Entries() ([]tui.ThemeEntry, error) {
	infos, err := tc.catalog.Entries()
	if err != nil {
		return nil, fmt.Errorf("theme catalog entries: %w", err)
	}
	entries := make([]tui.ThemeEntry, 0, len(infos))
...
func (tc *themeCatalog) Resolve(name string) (tui.ThemeSpec, bool) {
	th, ok := tc.catalog.Resolve(name)
	if !ok {
		return tui.ThemeSpec{}, false
	}
	return tui.ThemeSpec{
...
func (tc *themeCatalog) patchConfigTheme(themeName string) error {
	...
	// a config with stray theme lines is fully healed (not
