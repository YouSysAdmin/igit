...
func TestListThemesOutput(t *testing.T) {
	...
	assert.Equal(t, []string{"basic", "catppuccin-mocha", "default", "dracula", "gruvbox", "nord", "solarized-dark"}, names)
}
...
func TestResolveAutoThemeName(t *testing.T) {
	tests := []struct {
		...
		{name: "dark terminal uses default dark theme", darkBackground: true, want: "default"},
		{name: "light terminal uses default light theme", darkBackground: false, want: "basic"},
		{
...
func TestHandleThemes_NoColorsWarning(t *testing.T) {
	...
	require.NoError(t, cat.InitBundled())
	var stdout, stderr bytes.Buffer
	opts := options{Theme: "dracula", Display: displayOptions{NoColors: true}}

	done, err := handleThemes(&opts, cat, &stdout, &stderr)
	require.NoError(t, err)
	assert.False(t, done)
	assert.Contains(t, stderr.String(), "warning: --no-colors ignored when --theme is set")
	assert.False(t, opts.Display.NoColors, "--no-colors should be cleared")
...
func TestThemeCatalog_Entries(t *testing.T) {
	...
	assert.Equal(t, "default", entries[0].Name, "default theme should be first")
	for _, e := range entries {
...
// appending theme = ... after a trailing [color options] section
