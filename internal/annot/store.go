...
func (s *Store) Get(file string) []Annotation {
	...
	slices.SortFunc(result, func(a, b Annotation) int { return cmp.Compare(a.Line, b.Line) })
	return result
}
...
func (s *Store) All() map[string][]Annotation {
	...
	for file, anns := range s.annotations {
		...
		copy(copied, anns)
		slices.SortFunc(copied, func(a, b Annotation) int { return cmp.Compare(a.Line, b.Line) })
		result[file] = copied
	}
	return result
}
...
func (s *Store) Files() []string {
	return slices.Sorted(maps.Keys(s.annotations))
}
...
//
// A body line starting with "## " is written with one leading space so it
// cannot be read back as a record header. Other heading forms are left alone.
