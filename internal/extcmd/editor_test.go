...
func TestEditor_writeTempFile_WritesContent(t *testing.T) {
	...
	assert.Equal(t, "igit-annot-", filepath.Base(path)[:len("igit-annot-")])
...
func TestEditor_Command_WritesSeedAndBuildsCmd(t *testing.T) {
	...
	assert.True(t, strings.HasPrefix(filepath.Base(tempPath), "igit-annot-"))
...
func TestEditor_startsInItsDirectory(t *testing.T) {
	// an editor that reads its cwd on startup dies when igit's own directory is
	...
	e := Editor{Dir: dir}
	cmd, err := e.SourceCommand(file, 1)
	require.NoError(t, err)
	assert.Equal(t, dir, cmd.Dir, "the source editor starts in the repository")

	cmd, _, err = e.Command("seed")
	require.NoError(t, err)
	assert.Equal(t, dir, cmd.Dir, "so does the annotation editor")

	// the zero value keeps the old behavior of inheriting the caller's directory
	bare, err := Editor{}.SourceCommand(file, 1)
	require.NoError(t, err)
	assert.Empty(t, bare.Dir)
}
