...
// rename handling end-to-end across
...
func TestPathFilter(t *testing.T) {
	all := PathFilter(nil, nil)
	assert.True(t, all("anything/at/all.go"))

	inc := PathFilter([]string{"src/"}, nil)
	assert.True(t, inc("src/a.go"))
	assert.True(t, inc("src"))
	assert.False(t, inc("srcx/a.go"))

	both := PathFilter([]string{"src"}, []string{"src/vendor"})
	assert.True(t, both("src/a.go"))
	assert.False(t, both("src/vendor/x.go"))
	assert.False(t, both("docs/a.md"))
}

func TestGit_diffArgs_conflictAware(t *testing.T) {
	g := &Git{}
	// an unmerged path has no plain worktree diff, --ours turns it into one
	assert.Contains(t, g.diffArgs("", false), "--ours", "worktree diff")
	assert.NotContains(t, g.diffArgs("", true), "--ours", "--cached rejects --ours")
	assert.NotContains(t, g.diffArgs("HEAD~1", false), "--ours", "a ref diff rejects --ours")
	assert.NotContains(t, g.diffArgs("main..feature", false), "--ours")
}

func TestFoldUnmerged(t *testing.T) {
	// git reports an unmerged path twice, once as U and once as the change
	got := foldUnmerged([]FileEntry{
		{Path: "both.txt", Status: FileUnmerged},
		{Path: "both.txt", Status: FileModified},
		{Path: "new.txt", Status: FileAdded},
	})
	assert.Equal(t, []FileEntry{
		{Path: "both.txt", Status: FileUnmerged},
		{Path: "new.txt", Status: FileAdded},
	}, got, "the pair folds into one unmerged entry, in place")
	...
	assert.Equal(t, []FileEntry{{Path: "both.txt", Status: FileUnmerged}}, got)

	// a rename keeps its origin and is not folded away
	unchanged := []FileEntry{{Path: "b.go", OldPath: "a.go", Status: FileRenamed}, {Path: "c.go", Status: FileModified}}
	assert.Equal(t, unchanged, foldUnmerged(unchanged))
	assert.Empty(t, foldUnmerged(nil))
}

func TestGit_ChangedFiles_mergeConflict(t *testing.T) {
	dir := setupTestRepo(t)
	writeFile(t, dir, "both.txt", "alpha\nbeta\ngamma\n")
	gitCmd(t, dir, "add", "both.txt")
	gitCmd(t, dir, "commit", "-m", "base")
	gitCmd(t, dir, "checkout", "-b", "feature")
	writeFile(t, dir, "both.txt", "alpha\nfeature\ngamma\n")
	gitCmd(t, dir, "commit", "-am", "feature")
	gitCmd(t, dir, "checkout", "-")
	writeFile(t, dir, "both.txt", "alpha\nmain\ngamma\n")
	gitCmd(t, dir, "commit", "-am", "main")
	// the merge is expected to fail, gitCmd would flag that as an error
	...
	entries, err := g.ChangedFiles("", false)
	require.NoError(t, err)
	require.Len(t, entries, 1, "the conflicted file is listed once, not once per stage")
	assert.Equal(t, "both.txt", entries[0].Path)
	assert.Equal(t, FileUnmerged, entries[0].Status)

	// the diff is readable and carries the conflict markers
	lines, err := g.FileDiff(FileDiffRequest{Path: "both.txt", ContextLines: 3})
	require.NoError(t, err)
	var body strings.Builder
	for _, l := range lines {
		body.WriteString(l.Content + "\n")
	}
	assert.Contains(t, body.String(), "<<<<<<< HEAD")
	assert.Contains(t, body.String(), ">>>>>>> feature")
}
