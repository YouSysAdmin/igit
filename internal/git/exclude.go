...
// DiffSource is the local interface mirroring tui.DiffSource, exported for moq generation.
// contextLines controls how many lines of context surround each hunk in the diff output.
// 0 or values >= fullContextSentinel request full-file context (the igit default).
...
func NewExcludeFilter(inner DiffSource, prefixes []string) *ExcludeFilter {
