...
// FallbackSource wraps a diff source and knows about --only file paths.
...
// NewFallbackSource creates a FallbackSource that delegates to inner and falls back
// to reading files from disk for --only patterns not found in the diff.
func NewFallbackSource(inner DiffSource, only []string, workDir string) *FallbackSource {
	return &FallbackSource{inner: inner, only: only, workDir: workDir}
}
...
func (fr *FallbackSource) ChangedFiles(ref string, staged bool) ([]FileEntry, error) {
...
func (fr *FallbackSource) FileDiff(req FileDiffRequest) ([]DiffLine, error) {
	resolved := resolvePath(fr.workDir, req.Path)

	// skip inner renderer for files outside the repo - git would reject them
...
func (fr *FallbackSource) matchesAnyEntry(entries []FileEntry, pattern string) bool {
...
func (fr *FallbackSource) isOnlyFile(file string) bool {
	for _, pattern := range fr.only {
...
func (fr *FallbackSource) pathMatches(file, pattern string) bool {
...
func (fr *FallbackSource) isInsideWorkDir(absPath string) bool {
...
func (fr *FallbackSource) relativePath(absPath string) string {
	rel, err := filepath.Rel(fr.workDir, absPath)
...
// FileReader is a standalone DiffSource for use when no git repo is available.
