...
type fakeDiffer struct {
	fn func(req FileDiffRequest) ([]DiffLine, error)
}

func (f fakeDiffer) FileDiff(req FileDiffRequest) ([]DiffLine, error) {
	return f.fn(req)
}
...
func TestComputeStats_AggregatesAddsAndRemoves(t *testing.T) {
	...
	differ := fakeDiffer{fn: func(FileDiffRequest) ([]DiffLine, error) {
		return []DiffLine{
...
func TestComputeStats_RenamePassesOldPath(t *testing.T) {
	...
	entries := []FileEntry{{Path: "new.go", OldPath: "old.go", Status: FileRenamed}}
	var gotReq FileDiffRequest
	differ := fakeDiffer{fn: func(req FileDiffRequest) ([]DiffLine, error) {
		gotReq = req
		return []DiffLine{
			{ChangeType: ChangeRemove},
			{ChangeType: ChangeAdd},
...
func TestComputeStats_NonRenameHasEmptyOldPath(t *testing.T) {
	entries := []FileEntry{{Path: "a.go", Status: FileModified}}
	var gotReq FileDiffRequest
	differ := fakeDiffer{fn: func(req FileDiffRequest) ([]DiffLine, error) {
		gotReq = req
		return []DiffLine{{ChangeType: ChangeAdd}}, nil
	}}
	got := ComputeStats(StatsRequest{Differ: differ, Entries: entries})
...
func TestComputeStats_RenameStagedFallbackPassesOldPath(t *testing.T) {
	...
	entries := []FileEntry{{Path: "new.go", OldPath: "old.go", Status: FileAdded}}
	var stagedReq FileDiffRequest
	differ := fakeDiffer{fn: func(req FileDiffRequest) ([]DiffLine, error) {
		if !req.Staged {
			return nil, nil
		}
...
func TestComputeStats_FirstErrorStopsAndReturns(t *testing.T) {
	entries := []FileEntry{
		{Path: "good.go", Status: FileModified},
		{Path: "bad.go", Status: FileModified},
	}
...
func TestComputeStats_StagedFallbackForAddedFile(t *testing.T) {
	entries := []FileEntry{{Path: "new.go", Status: FileAdded}}
	differ := fakeDiffer{fn: func(req FileDiffRequest) ([]DiffLine, error) {
		if !req.Staged {
			return nil, nil
		}
		return []DiffLine{{ChangeType: ChangeAdd}, {ChangeType: ChangeAdd}}, nil
	}}
	got := ComputeStats(StatsRequest{Differ: differ, Entries: entries})
...
func TestComputeStats_UntrackedReadBypassesPrimaryDiffError(t *testing.T) {
	...
	entries := []FileEntry{{Path: rel, Status: FileUntracked}}
	called := false
	differ := fakeDiffer{fn: func(FileDiffRequest) ([]DiffLine, error) {
		called = true
...
func TestComputeStats_UntrackedReadOutsideWorkDirMarksPartial(t *testing.T) {
	...
	entries := []FileEntry{{Path: "../../etc/passwd", Status: FileUntracked}}
	differ := fakeDiffer{fn: func(FileDiffRequest) ([]DiffLine, error) {
		return nil, nil
	}}
...
func TestComputeStats_OversizedUntrackedFileSkipped(t *testing.T) {
	...
	entries := []FileEntry{{Path: "huge.bin", Status: FileUntracked}}
	differ := fakeDiffer{fn: func(FileDiffRequest) ([]DiffLine, error) {
		return nil, nil
	}}
	got := ComputeStats(StatsRequest{Differ: differ, WorkDir: root, Entries: entries})
...
func TestComputeStats_NonRegularUntrackedFileSkipped(t *testing.T) {
	...
	entries := []FileEntry{{Path: "pipe", Status: FileUntracked}}
	differ := fakeDiffer{fn: func(FileDiffRequest) ([]DiffLine, error) {
		return nil, nil
	}}
	got := ComputeStats(StatsRequest{Differ: differ, WorkDir: root, Entries: entries})
...
func TestComputeStats_BinaryUntrackedFileSkipped(t *testing.T) {
	...
	entries := []FileEntry{{Path: "blob.bin", Status: FileUntracked}}
	differ := fakeDiffer{fn: func(FileDiffRequest) ([]DiffLine, error) {
		return nil, nil
	}}
	got := ComputeStats(StatsRequest{Differ: differ, WorkDir: root, Entries: entries})
