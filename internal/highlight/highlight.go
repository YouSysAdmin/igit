...
// maxBacktrackingStack raises regexp2's default 100k-slot cap so that very
// long single-line tokens still highlight. One million slots are about 8 MB
// of runtrack on a 64-bit target, and runstack can add as much again. Chroma
// also puts a 250ms timeout on each lexer rule, so a large enough token still
...
// into that path, so raising the package default here reaches every lexer igit builds.
...
// HighlightLines takes a filename (for lexer detection) and a slice of git.DiffLine,
...
func (h *Highlighter) HighlightLines(filename string, lines []git.DiffLine) []string {
...
func (h *Highlighter) reconstructFiles(lines []git.DiffLine) (newFile, oldFile string) {
...
func (h *Highlighter) mapHighlightedLines(lines []git.DiffLine, newHL, oldHL []string) []string {
