...
func TestHighlighter_HighlightLines(t *testing.T) {
	...
	lines := []git.DiffLine{
		...
		{OldNum: 3, NewNum: 3, Content: `import "fmt"`, ChangeType: git.ChangeContext},
		{OldNum: 4, NewNum: 4, Content: "", ChangeType: git.ChangeContext},
		{NewNum: 5, Content: "func main() {", ChangeType: git.ChangeAdd},
		{NewNum: 6, Content: `	fmt.Println("hello")`, ChangeType: git.ChangeAdd},
		{NewNum: 7, Content: "}", ChangeType: git.ChangeAdd},
	}
...
func TestHighlighter_HighlightLinesWithRemoved(t *testing.T) {
	...
	lines := []git.DiffLine{
		{OldNum: 1, NewNum: 1, Content: "package main", ChangeType: git.ChangeContext},
		{OldNum: 2, Content: `var x = "old"`, ChangeType: git.ChangeRemove},
		{NewNum: 2, Content: `var x = "new"`, ChangeType: git.ChangeAdd},
		{OldNum: 3, NewNum: 3, Content: "", ChangeType: git.ChangeContext},
	}
...
func TestHighlighter_HighlightLinesWithDivider(t *testing.T) {
	...
	lines := []git.DiffLine{
		...
		{OldNum: 10, NewNum: 10, Content: `func foo() {}`, ChangeType: git.ChangeContext},
	}
...
func TestHighlighter_UnknownFileReturnsNil(t *testing.T) {
	h := New("monokai", true)
	lines := []git.DiffLine{
		{OldNum: 1, NewNum: 1, Content: "some data", ChangeType: git.ChangeContext},
	}
...
func TestHighlighter_EmptyLinesReturnsNil(t *testing.T) {
	...
	assert.Nil(t, h.HighlightLines("main.go", []git.DiffLine{}))
}
...
func TestHighlighter_ContextOnlyFile(t *testing.T) {
	...
	lines := []git.DiffLine{
		...
		{OldNum: 3, NewNum: 3, Content: `import "fmt"`, ChangeType: git.ChangeContext},
		{OldNum: 4, NewNum: 4, Content: "", ChangeType: git.ChangeContext},
		{OldNum: 5, NewNum: 5, Content: "func main() {", ChangeType: git.ChangeContext},
		{OldNum: 6, NewNum: 6, Content: `	fmt.Println("hello")`, ChangeType: git.ChangeContext},
		{OldNum: 7, NewNum: 7, Content: "}", ChangeType: git.ChangeContext},
	}
...
func TestReconstructFiles(t *testing.T) {
	lines := []git.DiffLine{
		{Content: "line1", ChangeType: git.ChangeContext},
		{Content: "removed", ChangeType: git.ChangeRemove},
		{Content: "added", ChangeType: git.ChangeAdd},
		{Content: "~~~", ChangeType: git.ChangeDivider},
		{Content: "line2", ChangeType: git.ChangeContext},
	}
...
func TestHighlighter_LongQuotedStringUsesStringColor(t *testing.T) {
	// a long Go quoted string must keep its string color, not fall back to error
	...
	render := func(n int) string {
		...
		got := h.HighlightLines("main.go", []git.DiffLine{{NewNum: 1, Content: content, ChangeType: git.ChangeContext}})
		require.Len(t, got, 1)
		return got[0]
	}
