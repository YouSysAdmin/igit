...
func TestNormalizeKey_metaEnterIsAltEnter(t *testing.T) {
	// macOS terminals label the alt key Meta, bubbletea reports it as alt
	assert.Equal(t, "alt+enter", normalizeKey("meta+enter"))
	assert.Equal(t, "alt+enter", normalizeKey("Meta+Enter"))
}
...
// tmux copy-mode) and swallow it before igit sees the key.
...
func TestUnbind(t *testing.T) {
	...
	assert.Equal(t, ActionQuitDiscarding, km.Resolve("Q"), "Q should still map to discard_quit")
}
...
func TestActionFlushOutput_HelpEntry(t *testing.T) {
	...
	for _, e := range entries {
		if e.Action == ActionFlushOutput {
			assert.Equal(t, "flush annotations to the output file", e.Description)
			assert.Equal(t, "Annotations", e.Section)
			found = true
			break
		}
...
func TestResolveChord_LayoutFallback(t *testing.T) {
	// Cyrillic che sits on the same physical key as x on QWERTY.
	// a chord bound under the latin "x" must still resolve when the user presses che.
	km := Default()
...
func TestDisplayKey(t *testing.T) {
	assert.Equal(t, "space", DisplayKey(" "))
	assert.Equal(t, "g>space", DisplayKey("g> "))
	assert.Equal(t, "alt+g", DisplayKey("alt+g"))
}

func TestHelpSections_namesTheSpaceBar(t *testing.T) {
	// the binding is stored as " ", which would render as an empty key column
	for name, km := range map[string]*Keymap{"review": Default(), "commit": DefaultCommit()} {
		var found bool
		for _, section := range km.HelpSections() {
			for _, e := range section.Entries {
				assert.NotEmpty(t, strings.TrimSpace(e.Keys), "%s: %s has a blank key column", name, e.Action)
				if strings.Contains(e.Keys, "space") {
					found = true
				}
			}
		}
		assert.True(t, found, "%s: the space bar is named in the help", name)
	}
}
