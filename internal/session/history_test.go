...
func TestHistoryDir_Default(t *testing.T) {
	...
	assert.Equal(t, filepath.Join(home, ".config", "igit", "history", "myrepo"), got)
}
...
func TestSave_FileLocationAndFormat(t *testing.T) {
	...
	// one file per review scope: a working-tree review is "worktree.md"
	files, err := os.ReadDir(filepath.Join(histDir, "myproject"))
	require.NoError(t, err)
	require.Len(t, files, 1)
	assert.Equal(t, "worktree.md", files[0].Name())
...
func TestSave_FilenameIsTheScope(t *testing.T) {
	histDir := t.TempDir()
	New(histDir).Save(Params{Annotations: "## file.go:1 (+)\nnote\n", Path: "/myrepo", Ref: "origin/main..topic"})

	files, err := os.ReadDir(filepath.Join(histDir, "myrepo"))
	require.NoError(t, err)
	require.Len(t, files, 1)
	assert.Equal(t, "ref-origin_main..topic.md", files[0].Name())
}
...
func TestSave_NamedCommitWinsOverHead(t *testing.T) {
	gitRoot := t.TempDir()
	setupGitRepo(t, gitRoot)
	histDir := t.TempDir()
	New(histDir).Save(Params{
	...
	entries := readHistoryFiles(t, histDir)
	require.Len(t, entries, 1)
	assert.Contains(t, entries[0], "commit: abc1234")
}

func TestScopeName_requestKinds(t *testing.T) {
	assert.Equal(t, "pr-7", ScopeName(Params{PR: 7}), "empty kind keeps the pull request prefix")
	assert.Equal(t, "pr-7", ScopeName(Params{PR: 7, PRKind: "pr"}))
	assert.Equal(t, "mr-7", ScopeName(Params{PR: 7, PRKind: "mr"}))
}
