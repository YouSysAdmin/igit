...
// loadGalleryOnce parses the embedded gallery once per process. The parser
// reads no Catalog state, so a zero Catalog can drive it.
var loadGalleryOnce = sync.OnceValues(func() (map[string]Theme, error) {
	return (&Catalog{}).loadGallery()
})
...
func (c *Catalog) loadGallery() (map[string]Theme, error) {
	...
	for _, e := range entries {
		...
		data, err := fs.ReadFile(galleryFS, galleryDir+"/"+e.Name())
		if err != nil {
...
func (c *Catalog) galleryNames() ([]string, error) {
	...
	return slices.Sorted(maps.Keys(gal)), nil
}
