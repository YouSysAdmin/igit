...
func TestPickAdjacentAnnotation(t *testing.T) {
	a1 := annot.Annotation{File: "a.go", Line: 5, Type: "+"}
	a2 := annot.Annotation{File: "a.go", Line: 10, Type: "+"}
	b1 := annot.Annotation{File: "b.go", Line: 0, Type: ""}
	b2 := annot.Annotation{File: "b.go", Line: 7, Type: "-"}
	flat := []annot.Annotation{a1, a2, b1, b2}

	tests := []struct {
		name    string
		flat    []annot.Annotation
		cur     cursorAnnotKey
		forward bool
		want    annot.Annotation
		ok      bool
	}{
		{"empty list forward", nil, cursorAnnotKey{file: "a.go", line: 1}, true, annot.Annotation{}, false},
		{"empty list backward", nil, cursorAnnotKey{file: "a.go", line: 1}, false, annot.Annotation{}, false},

		{"forward from before first", flat, cursorAnnotKey{file: "a.go", line: 1}, true, a1, true},
		{"backward from before first → no-op", flat, cursorAnnotKey{file: "a.go", line: 1}, false, annot.Annotation{}, false},

		{"forward from on first annotation", flat, cursorAnnotKey{file: "a.go", line: 5, typ: "+", onAnnot: true}, true, a2, true},
		{"backward from on first annotation → no-op", flat, cursorAnnotKey{file: "a.go", line: 5, typ: "+", onAnnot: true}, false, annot.Annotation{}, false},
		...
		{"forward from on last annotation → no-op", flat, cursorAnnotKey{file: "b.go", line: 7, typ: "-", onAnnot: true}, true, annot.Annotation{}, false},
		{"backward from on last annotation", flat, cursorAnnotKey{file: "b.go", line: 7, typ: "-", onAnnot: true}, false, b1, true},

		{"forward past last → no-op", flat, cursorAnnotKey{file: "z.go", line: 1}, true, annot.Annotation{}, false},
		...
		{"forward from context at same line as add-annotation skips same-line",
			...
			annot.Annotation{File: "a.go", Line: 9, Type: "+"}, true,
		},
		{"backward from context at same line as add-annotation reaches it",
			...
			annot.Annotation{File: "a.go", Line: 5, Type: "+"}, true,
		},
...
func TestModel_CurrentAnnotKey(t *testing.T) {
	lines := []git.DiffLine{
		...
		{ChangeType: git.ChangeRemove, Content: "removed", OldNum: 2, NewNum: 0},
	}
	...
	t.Run("onAnnot true when store has matching annotation", func(t *testing.T) {
		m.store.Add(annot.Annotation{File: "a.go", Line: 2, Type: "+", Comment: "x"})
		m.nav.diffCursor = 1
		key := m.currentAnnotKey()
	...
	t.Run("leading ChangeDivider collapses to line -1 so file-level annot stays reachable", func(t *testing.T) {
		...
		divLines := []git.DiffLine{
			{ChangeType: git.ChangeDivider, Content: "⋯ 5 lines ⋯", OldNum: 0, NewNum: 0},
			{ChangeType: git.ChangeAdd, Content: "added", OldNum: 0, NewNum: 6},
		}
	...
	t.Run("middle ChangeDivider inherits position from preceding non-divider line", func(t *testing.T) {
		...
		divLines := []git.DiffLine{
			{ChangeType: git.ChangeAdd, Content: "first", OldNum: 0, NewNum: 5},
			{ChangeType: git.ChangeDivider, Content: "⋯ 10 lines ⋯", OldNum: 0, NewNum: 0},
			{ChangeType: git.ChangeAdd, Content: "later", OldNum: 0, NewNum: 16},
		}
		dm := testModel([]string{"a.go"}, nil)
		dm.file.name = "a.go"
		dm.file.lines = divLines
	...
	t.Run("trailing ChangeDivider inherits position from preceding non-divider line", func(t *testing.T) {
		divLines := []git.DiffLine{
			{ChangeType: git.ChangeAdd, Content: "added", OldNum: 0, NewNum: 5},
			{ChangeType: git.ChangeDivider, Content: "⋯ 3 lines ⋯", OldNum: 0, NewNum: 0},
		}
		dm := testModel([]string{"a.go"}, nil)
		dm.file.name = "a.go"
		dm.file.lines = divLines
	...
	t.Run("consecutive dividers walk back through them to reach a non-divider", func(t *testing.T) {
		divLines := []git.DiffLine{
			{ChangeType: git.ChangeAdd, Content: "added", OldNum: 0, NewNum: 5},
			{ChangeType: git.ChangeDivider, Content: "⋯ 3 lines ⋯", OldNum: 0, NewNum: 0},
			{ChangeType: git.ChangeDivider, Content: "⋯ 2 lines ⋯", OldNum: 0, NewNum: 0},
		}
		dm := testModel([]string{"a.go"}, nil)
		dm.file.name = "a.go"
		dm.file.lines = divLines
...
func TestModel_HandleAnnotNav_FromMiddleDivider(t *testing.T) {
	lines := []git.DiffLine{
		{ChangeType: git.ChangeAdd, Content: "L5", OldNum: 0, NewNum: 5},
		{ChangeType: git.ChangeDivider, Content: "⋯ 10 lines ⋯", OldNum: 0, NewNum: 0},
		{ChangeType: git.ChangeAdd, Content: "L16", OldNum: 0, NewNum: 16},
		{ChangeType: git.ChangeAdd, Content: "L17", OldNum: 0, NewNum: 17},
	}
	...
	m.store.Add(annot.Annotation{File: "a.go", Line: 5, Type: "+", Comment: "before"})
	m.store.Add(annot.Annotation{File: "a.go", Line: 17, Type: "+", Comment: "after"})
...
func TestModel_HandleAnnotNav_WithinFile(t *testing.T) {
	...
	lines := []git.DiffLine{
		{ChangeType: git.ChangeContext, Content: "ctx", OldNum: 1, NewNum: 1},
		{ChangeType: git.ChangeAdd, Content: "a", OldNum: 0, NewNum: 2},
		{ChangeType: git.ChangeAdd, Content: "b", OldNum: 0, NewNum: 3},
		{ChangeType: git.ChangeAdd, Content: "c", OldNum: 0, NewNum: 4},
	}
	m := testModel([]string{"a.go"}, map[string][]git.DiffLine{"a.go": lines})
	result, _ := m.Update(filesLoadedMsg{entries: []git.FileEntry{{Path: "a.go"}}})
	m = result.(Model)
	...
	result, _ = m.Update(loadMsg)
	m = result.(Model)
	m.store.Add(annot.Annotation{File: "a.go", Line: 2, Type: "+", Comment: "first"})
	m.store.Add(annot.Annotation{File: "a.go", Line: 4, Type: "+", Comment: "second"})
...
func TestModel_HandleAnnotNav_CrossFile(t *testing.T) {
	diffs := map[string][]git.DiffLine{
		"a.go": {{ChangeType: git.ChangeAdd, Content: "a1", OldNum: 0, NewNum: 1}},
		"b.go": {{ChangeType: git.ChangeAdd, Content: "b1", OldNum: 0, NewNum: 1}},
	}
	...
	result, _ = m.Update(loadMsg)
	m = result.(Model)
	m.store.Add(annot.Annotation{File: "a.go", Line: 1, Type: "+", Comment: "in a"})
	m.store.Add(annot.Annotation{File: "b.go", Line: 1, Type: "+", Comment: "in b"})
...
func TestModel_HandleAnnotNav_DeletionRobustness(t *testing.T) {
	lines := []git.DiffLine{
		{ChangeType: git.ChangeAdd, Content: "a", OldNum: 0, NewNum: 1},
		{ChangeType: git.ChangeAdd, Content: "b", OldNum: 0, NewNum: 2},
		{ChangeType: git.ChangeAdd, Content: "c", OldNum: 0, NewNum: 3},
	}
	m := testModel([]string{"a.go"}, map[string][]git.DiffLine{"a.go": lines})
	result, _ := m.Update(filesLoadedMsg{entries: []git.FileEntry{{Path: "a.go"}}})
	m = result.(Model)
	...
	t.Run("delete current annotation then forward lands on next", func(t *testing.T) {
		m.store.Clear()
		m.store.Add(annot.Annotation{File: "a.go", Line: 1, Type: "+", Comment: "x"})
		m.store.Add(annot.Annotation{File: "a.go", Line: 3, Type: "+", Comment: "y"})
...
func TestModel_HandleAnnotNav_KeymapDispatch(t *testing.T) {
	...
	m = result.(Model)
	m.store.Add(annot.Annotation{File: "a.go", Line: 1, Type: "+", Comment: "x"})
	m.store.Add(annot.Annotation{File: "a.go", Line: 2, Type: "+", Comment: "y"})
...
func TestModel_HandleAnnotNav_CollapsedModeExpandsHunk(t *testing.T) {
	...
	lines := []git.DiffLine{
		...
		{ChangeType: git.ChangeContext, Content: "ctx2", OldNum: 2, NewNum: 2},
		{ChangeType: git.ChangeRemove, Content: "old", OldNum: 3, NewNum: 0},
		{ChangeType: git.ChangeAdd, Content: "new", OldNum: 0, NewNum: 3},
		{ChangeType: git.ChangeContext, Content: "ctx3", OldNum: 4, NewNum: 4},
	}
	m := testModel([]string{"a.go"}, map[string][]git.DiffLine{"a.go": lines})
	result, _ := m.Update(filesLoadedMsg{entries: []git.FileEntry{{Path: "a.go"}}})
	m = result.(Model)
	...
	m.store.Add(annot.Annotation{File: "a.go", Line: 3, Type: "-", Comment: "removed"})
...
// crossing a file boundary backwards triggers the same pendingAnnotJump
// handoff and loadSelectedIfChanged path as forwards.
...
func TestModel_HandleAnnotNav_SkipsNonJumpableSameFileTarget(t *testing.T) {
	...
	lines := []git.DiffLine{
		{ChangeType: git.ChangeAdd, Content: "L5", OldNum: 0, NewNum: 5},
		{ChangeType: git.ChangeAdd, Content: "L9", OldNum: 0, NewNum: 9},
	}
	m := testModel([]string{"a.go"}, map[string][]git.DiffLine{"a.go": lines})
	result, _ := m.Update(filesLoadedMsg{entries: []git.FileEntry{{Path: "a.go"}}})
	m = result.(Model)
	...
	m.store.Add(annot.Annotation{File: "a.go", Line: 5, Type: "+", Comment: "a"})
	m.store.Add(annot.Annotation{File: "a.go", Line: 7, Type: "+", Comment: "stuck"})
	m.store.Add(annot.Annotation{File: "a.go", Line: 9, Type: "+", Comment: "c"})
...
// comment sub-row (nav.onAnnotationRow), not on the diff line above it.
// The diff line and the comment sub-row render as separate visual rows. without
// onAnnotationRow the highlight sits one row above the comment, which is
...
func TestModel_HandleAnnotNav_CursorLandsOnAnnotationRow(t *testing.T) {
	lines := []git.DiffLine{
		{ChangeType: git.ChangeAdd, Content: "L1", OldNum: 0, NewNum: 1},
		{ChangeType: git.ChangeAdd, Content: "L2", OldNum: 0, NewNum: 2},
		{ChangeType: git.ChangeAdd, Content: "L3", OldNum: 0, NewNum: 3},
	}
	m := testModel([]string{"a.go"}, map[string][]git.DiffLine{"a.go": lines})
	result, _ := m.Update(filesLoadedMsg{entries: []git.FileEntry{{Path: "a.go"}}})
	m = result.(Model)
	...
	result, _ = m.Update(loadMsg)
	m = result.(Model)
	m.store.Add(annot.Annotation{File: "a.go", Line: 3, Type: "+", Comment: "target"})

	t.Run("} sets onAnnotationRow when landing on a line-level annotation", func(t *testing.T) {
		m.nav.diffCursor = 0
		m.nav.onAnnotationRow = false
		result, _ := m.handleAnnotNav(true)
		model := result.(Model)
		assert.Equal(t, 2, model.nav.diffCursor, "cursor must be on the annotated diff line")
		assert.True(t, model.nav.onAnnotationRow, "cursor must render on the annotation comment sub-row, not the diff line above it")
	})
	...
	t.Run("{ sets onAnnotationRow when landing on a line-level annotation", func(t *testing.T) {
		...
		m.store.Add(annot.Annotation{File: "a.go", Line: 1, Type: "+", Comment: "earlier"})
		result, _ := m.handleAnnotNav(false)
		model := result.(Model)
		assert.Equal(t, 0, model.nav.diffCursor)
		assert.True(t, model.nav.onAnnotationRow, "cursor must render on the annotation comment sub-row")
	})
}
...
func TestModel_JumpToAnnotationTarget_CursorLandsOnAnnotationRow(t *testing.T) {
	...
	result, _ = m.Update(loadMsg)
	m = result.(Model)
	m.store.Add(annot.Annotation{File: "a.go", Line: 2, Type: "+", Comment: "popup target"})

	m.nav.onAnnotationRow = false
	result, _ = m.jumpToAnnotationTarget(&overlay.AnnotationTarget{File: "a.go", ChangeType: "+", Line: 2})
	model := result.(Model)
	assert.Equal(t, 1, model.nav.diffCursor)
	assert.True(t, model.nav.onAnnotationRow, "@ popup selection must land cursor on annotation comment row")
}

// file-level annotations use diffCursor=-1 which already represents the
// annotation row directly. onAnnotationRow must NOT be set in that case
...
func TestModel_HandleAnnotNav_FileLevelDoesNotSetCursorOnAnnotation(t *testing.T) {
	...
	result, _ = m.Update(loadMsg)
	m = result.(Model)
	m.store.Add(annot.Annotation{File: "a.go", Line: 0, Type: "", Comment: "file-level"})
	...
	assert.False(t, model.nav.onAnnotationRow, "file-level annotation row uses diffCursor=-1, not the onAnnotationRow flag")
}
