...
func (m Model) renderCollapsedDiff() string {
	...
	for i, dl := range m.file.lines {
		...
		case git.ChangeAdd:
			if expanded {
				m.renderDiffLine(&b, i, dl, dec) // use standard add styling when hunk is expanded
			} else {
		...
		m.renderAnnotationOrInput(&b, i, dec)
	}
...
func (m Model) maxRenderedContentWidth() int {
	...
	for i, dl := range m.file.lines {
		...
		hunkStart := -1
		if (dl.ChangeType == git.ChangeAdd || dl.ChangeType == git.ChangeRemove) && len(hunks) > 0 && hunks[hunkIdx] <= i {
			hunkStart = hunks[hunkIdx]
		}
		expanded := hunkStart >= 0 && m.modes.collapsed.expandedHunks[hunkStart]

		if dl.ChangeType == git.ChangeRemove && !expanded {
...
func (m Model) renderCollapsedAddLine(b *strings.Builder, idx int, dl git.DiffLine, modified bool) {
	...
	prefixFg := m.resolver.LineFg(git.ChangeAdd)
	if modified {
...
// renderWrappedCollapsedLine renders a collapsed add line with word wrapping, producing continuation lines with a wrap markers.
...
// the first row uses ctx.gutter. continuation rows prepend a muted wrap marker so it
...
func (m Model) deletePlaceholderText(hunkStart int) string {
	...
	for i := hunkStart; i < len(m.file.lines); i++ {
		...
		if ct == git.ChangeContext || ct == git.ChangeDivider {
			break
		}
		if ct == git.ChangeRemove {
			count++
		}
	}
	if count == 1 {
...
// shows the "N lines deleted" placeholder with remove styling so users know deletions exist and can expand with '.'.
...
func (m Model) renderDeletePlaceholder(b *strings.Builder, idx, hunkStart int) {
	...
	divider := git.DiffLine{ChangeType: git.ChangeDivider}
...
func (m Model) buildModifiedSet(hunks []int) map[int]bool {
	...
	for hi, start := range hunks {
		...
		for end > start && (m.file.lines[end-1].ChangeType != git.ChangeAdd &&
			m.file.lines[end-1].ChangeType != git.ChangeRemove) {
			end--
		}
		...
		if len(pairs) > 0 {
			for i := start; i < end; i++ {
				if m.file.lines[i].ChangeType == git.ChangeAdd {
					result[i] = true
				}
			}
		}
...
func (m *Model) toggleCollapsedMode() {
	...
	m.nav.onAnnotationRow = false // visible lines change, reset annotation cursor state
...
func (m *Model) toggleHunkExpansion() {
	...
	if m.modes.collapsed.expandedHunks[hunkStart] {
		...
		m.nav.onAnnotationRow = false // annotations on removed lines become invisible
		m.adjustCursorIfHidden()
		m.realignSearchCursor()
	} else {
...
func (m Model) isCollapsedHidden(idx int, hunks []int) bool {
	...
	if m.file.lines[idx].ChangeType != git.ChangeRemove {
		return false
	}
...
func (m Model) isDeleteOnlyPlaceholder(idx int, hunks []int) bool {
	...
	if idx < 0 || idx >= len(m.file.lines) || m.file.lines[idx].ChangeType != git.ChangeRemove {
		return false
	}
	hunkStart := m.hunkStartFor(idx, hunks)
...
func (m Model) firstVisibleInHunk(hunkStart int, hunks []int) int {
	...
	for i := hunkStart + 1; i < len(m.file.lines); i++ {
		if m.file.lines[i].ChangeType == git.ChangeDivider || m.file.lines[i].ChangeType == git.ChangeContext {
