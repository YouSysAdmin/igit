...
func TestModel_UnreviewedFilterAdvancesAsFilesAreReviewed(t *testing.T) {
	lines := map[string][]git.DiffLine{
		"a.go": {{NewNum: 1, Content: "a", ChangeType: git.ChangeAdd}},
		"b.go": {{NewNum: 1, Content: "b", ChangeType: git.ChangeAdd}},
	}
...
func TestModel_MarkReviewedDropsAsyncResultForPathRemovedDuringReload(t *testing.T) {
	...
	result, _ = model.Update(filesLoadedMsg{
		...
		entries: []git.FileEntry{{Path: "a.go"}},
	})
	model = result.(Model)
...
func TestModel_MarkReviewedKeepsAsyncResultForPathSurvivingReload(t *testing.T) {
	...
	result, _ = model.Update(filesLoadedMsg{
		seq:     model.filesLoadSeq,
		entries: []git.FileEntry{{Path: "a.go"}, {Path: "b.go"}},
	})
	model = result.(Model)
...
func TestModel_FKeyFilterToggle(t *testing.T) {
	...
	m.tree = testNewFileTree([]string{"a.go", "b.go"})
	m.store.Add(annot.Annotation{File: "a.go", Line: 1, Type: "+", Comment: "test annotation"})
...
func TestModel_FilterToggleLoadsDiffForNewSelection(t *testing.T) {
	lines := map[string][]git.DiffLine{
		"a.go": {{NewNum: 1, Content: "a-line", ChangeType: git.ChangeAdd}},
		"b.go": {{NewNum: 1, Content: "b-line", ChangeType: git.ChangeAdd}},
	}
	m := testModel([]string{"a.go", "b.go"}, lines)
	m.tree = testNewFileTree([]string{"a.go", "b.go"})
	m.file.name = "b.go"
	...
	// annotate only a.go
	m.store.Add(annot.Annotation{File: "a.go", Line: 1, Type: "+", Comment: "note on a"})
...
func TestModel_QuitPreservesAnnotations(t *testing.T) {
	m := testModel([]string{"a.go", "b.go"}, nil)
	m.store.Add(annot.Annotation{File: "a.go", Line: 5, Type: "+", Comment: "needs review"})
	m.store.Add(annot.Annotation{File: "b.go", Line: 10, Type: " ", Comment: "check this"})
...
func TestModel_NoConfirmDiscardWired(t *testing.T) {
	...
	assert.True(t, m.session.noConfirmDiscard, "noConfirmDiscard should be wired from ModelConfig")
}
...
func TestModel_HandleEscKeyClearsSearch(t *testing.T) {
	m := testModel([]string{"a.go"}, map[string][]git.DiffLine{
		"a.go": {{ChangeType: git.ChangeAdd, Content: "hello world"}},
	})
	m.file.name = "a.go"
	m.file.lines = []git.DiffLine{{ChangeType: git.ChangeAdd, Content: "hello world"}}
	m.search.term = "hello"
...
func TestModel_HandleInfo_OpensOverlayWhenApplicable(t *testing.T) {
	commits := []git.CommitInfo{{Hash: "abc123"}, {Hash: "def456"}}
	m := testModel([]string{"a.go"}, nil)
...
func TestModel_HandleInfo_OpensOverlayEvenWhenCommitsHidden(t *testing.T) {
	// the overlay must open in modes
	...
	fake := &fakeCommitLog{fn: func(string) ([]git.CommitInfo, error) {
		return []git.CommitInfo{{Hash: "abc"}}, nil
	}}
	m := testModel([]string{"a.go"}, nil)
...
func TestModel_HandleInfo_OpensImmediatelyDuringLoad(t *testing.T) {
	// pressing `i` before commits finish loading must open the
...
func TestModel_ActionReload_StdinGuard(t *testing.T) {
	...
	model := result.(Model)
	assert.Equal(t, "Reload not available in this session", model.reload.hint)
...
func TestModel_ActionReload_NoAnnotations_DirectReload(t *testing.T) {
	...
	renderer := &mocks.DiffSourceMock{
		ChangedFilesFunc: func(ref string, staged bool) ([]git.FileEntry, error) {
			callCount++
			return []git.FileEntry{{Path: "a.go"}}, nil
		},
		FileDiffFunc: func(git.FileDiffRequest) ([]git.DiffLine, error) { return nil, nil },
	}
	m := testNewModel(t, renderer, annot.NewStore(), noopHighlighter(),
...
func TestModel_ActionReload_NoConfirmReload_DirectReload(t *testing.T) {
	...
	m := testNewModel(t, renderer, store, noopHighlighter(),
		ModelConfig{Applicable: Applicable{Reload: true}, NoConfirmReload: true})
	require.True(t, m.session.noConfirmReload, "noConfirmReload must be wired from ModelConfig")
...
func TestModel_ToggleCompactMode_PreservesCursorAcrossToggle(t *testing.T) {
	...
	fullDiff := []git.DiffLine{
		{NewNum: 38, Content: "a", ChangeType: git.ChangeContext},
		{NewNum: 39, Content: "b", ChangeType: git.ChangeContext},
		{NewNum: 40, Content: "context before", ChangeType: git.ChangeContext},
		{NewNum: 41, Content: "added line", ChangeType: git.ChangeAdd},
		{NewNum: 42, Content: "context after", ChangeType: git.ChangeContext},
	}
	// compact view of the same file: the added line moves to index 2
	compactDiff := []git.DiffLine{
	...
	renderer := &mocks.DiffSourceMock{
		ChangedFilesFunc: func(string, bool) ([]git.FileEntry, error) { return nil, nil },
		FileDiffFunc:     func(git.FileDiffRequest) ([]git.DiffLine, error) { return compactDiff, nil },
	}
	m := testModel([]string{"a.go"}, nil)
	m.diffSource = renderer
	m.compact.applicable = true
...
func TestModel_ToggleCompactMode_FallsBackToHunkWhenAnchorLineDropped(t *testing.T) {
	...
	fullDiff := []git.DiffLine{
		{NewNum: 3, Content: "x", ChangeType: git.ChangeContext},
		{NewNum: 4, Content: "y", ChangeType: git.ChangeContext},
		{NewNum: 5, Content: "far context", ChangeType: git.ChangeContext},
		{NewNum: 6, Content: "z", ChangeType: git.ChangeContext},
		{NewNum: 40, Content: "hunk add", ChangeType: git.ChangeAdd},
	}
	// compact view: the far context line (NewNum 5) is gone. only the hunk remains
	compactDiff := []git.DiffLine{
		{ChangeType: git.ChangeDivider},
		{NewNum: 39, Content: "context", ChangeType: git.ChangeContext},
		{NewNum: 40, Content: "hunk add", ChangeType: git.ChangeAdd},
	}
	renderer := &mocks.DiffSourceMock{
		ChangedFilesFunc: func(string, bool) ([]git.FileEntry, error) { return nil, nil },
...
func TestModel_ToggleCompactMode_PreservesCursorCompactToFull(t *testing.T) {
	...
	renderer := &mocks.DiffSourceMock{
		ChangedFilesFunc: func(string, bool) ([]git.FileEntry, error) { return nil, nil },
		FileDiffFunc:     func(git.FileDiffRequest) ([]git.DiffLine, error) { return fullDiff, nil },
	}
	m := testModel([]string{"a.go"}, nil)
	m.diffSource = renderer
	m.compact.applicable = true
...
func TestBuildHelpSpec_StatusIconsOnToggleRows(t *testing.T) {
	...
	tests := []struct {
		...
		{"View", "untracked files: show or hide", "∅ untracked files: show or hide"},
...
func TestModel_HelpOverlayScrollsOnSmallTerminal(t *testing.T) {
	...
	top := m.View()
	require.NotContains(t, top, "quit, discarding the annotations", "last section must start below the fold at 100x40")
	...
	assert.Contains(t, view, "quit, discarding the annotations", "paging must reach the last section")
