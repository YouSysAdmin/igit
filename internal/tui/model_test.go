...
// plainRenderer returns a DiffSourceMock that returns an empty file list and no diff lines.
func plainRenderer() *mocks.DiffSourceMock {
	return &mocks.DiffSourceMock{
...
// testFileTreeFactory returns the sidepane factory closure for NewFileTree,
// suitable for injection into ModelConfig in tests.
func testFileTreeFactory() func(entries []git.FileEntry) FileTreeComponent {
	return func(entries []git.FileEntry) FileTreeComponent {
...
// (PlainResolver + its derived DiffSource + a zero SGR) and sidepane factories
// when the cfg doesn't set them explicitly.
func testNewModel(t *testing.T, renderer DiffSource, store *annot.Store, highlighter SyntaxHighlighter, cfg ModelConfig) Model {
	t.Helper()
	cfg.DiffSource = renderer
	cfg.Store = store
...
func testModel(files []string, fileDiffs map[string][]git.DiffLine) Model {
	entries := make([]git.FileEntry, len(files))
	for i, f := range files {
		entries[i] = git.FileEntry{Path: f}
	}
...
func TestNewModel_RequiredDependencies(t *testing.T) {
	...
	validCfg := func() ModelConfig {
		return ModelConfig{
			DiffSource:    &mocks.DiffSourceMock{},
			Store:         annot.NewStore(),
			Highlighter:   noopHighlighter(),
	...
	tests := []struct {
		...
		{name: "nil DiffSource", patch: func(c *ModelConfig) { c.DiffSource = nil }},
...
func TestNewModel_OptionalDefaults(t *testing.T) {
	...
	t.Run("custom keymap is used when provided", func(t *testing.T) {
		km := keymap.Default()
		km.Unbind("q")
		m := testNewModel(t, renderer, annot.NewStore(), noopHighlighter(), ModelConfig{Keymap: km})
		action := m.keymap.Resolve("q")
		assert.Equal(t, keymap.Action(""), action)
	})

	t.Run("TreeWidthRatio below range defaults to 2", func(t *testing.T) {
		m := testNewModel(t, renderer, annot.NewStore(), noopHighlighter(), ModelConfig{TreeWidthRatio: 0})
		assert.Equal(t, 2, m.session.treeWidthRatio)
	})

	t.Run("TreeWidthRatio above range defaults to 2", func(t *testing.T) {
		m := testNewModel(t, renderer, annot.NewStore(), noopHighlighter(), ModelConfig{TreeWidthRatio: 15})
		assert.Equal(t, 2, m.session.treeWidthRatio)
	})

	t.Run("TreeWidthRatio in range is kept", func(t *testing.T) {
		m := testNewModel(t, renderer, annot.NewStore(), noopHighlighter(), ModelConfig{TreeWidthRatio: 5})
		assert.Equal(t, 5, m.session.treeWidthRatio)
	})
}
...
func TestModel_Init(t *testing.T) {
	...
	require.True(t, ok)
	assert.Equal(t, []string{"a.go", "b.go"}, git.FileEntryPaths(flm.entries))
	assert.NoError(t, flm.err)
}

func TestModel_InitialLoadingState_NoEmptyFlash(t *testing.T) {
	// before filesLoadedMsg is handled, View() must not paint the two-pane
	// layout with an empty tree and "no file selected". That made users see a 100-500 ms
	// flash of a "no changes" screen on launch. See internal/tui/view.go and handleFilesLoaded.
	m := testModel([]string{"a.go", "b.go"}, nil)
...
func TestModel_CompactModeFromConfig(t *testing.T) {
	...
	t.Run("compact enabled and applicable via config", func(t *testing.T) {
		m := testNewModel(t, renderer, store, noopHighlighter(), ModelConfig{
			Compact: true, CompactContext: 10, Applicable: Applicable{Compact: true}, TreeWidthRatio: 2,
		})
	...
	t.Run("compact requested but not applicable drops to off at construction", func(t *testing.T) {
		m := testNewModel(t, renderer, store, noopHighlighter(), ModelConfig{
			Compact: true, CompactContext: 7, Applicable: Applicable{Compact: false}, TreeWidthRatio: 2,
		})
...
func TestModel_BlameFromConfig(t *testing.T) {
	...
	blamer := &mocks.BlamerMock{
		FileBlameFunc: func(string, string, bool) (map[int]git.BlameLine, error) { return map[int]git.BlameLine{}, nil },
	}
...
func TestModel_TreeWidthRatio(t *testing.T) {
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			m := testNewModel(t, renderer, annot.NewStore(), noopHighlighter(), ModelConfig{TreeWidthRatio: tc.ratio})
...
func TestModel_CustomKeymapViewToggle(t *testing.T) {
	...
	lines := []git.DiffLine{{NewNum: 1, Content: "ctx", ChangeType: git.ChangeContext}}
	m := testModel([]string{"a.go"}, map[string][]git.DiffLine{"a.go": lines})
	m.keymap = km
	m.layout.focus = paneDiff
	m.file.name = "a.go"
...
// fakeCommitLog is a tiny test fake satisfying both git.CommitLogger and the
...
type fakeCommitLog struct {
	fn      func(ref string) ([]git.CommitInfo, error)
	calls   int
	lastRef string
	allRefs []string
}

func (f *fakeCommitLog) CommitLog(ref string) ([]git.CommitInfo, error) {
	f.calls++
	f.lastRef = ref
...
// rendererWithCommitLog wraps DiffSourceMock and adds a CommitLog method so the
// renderer satisfies git.CommitLogger. Used to verify the type-assertion
// fallback path in NewModel.
type rendererWithCommitLog struct {
	*mocks.DiffSourceMock
	commit *fakeCommitLog
}

func (r rendererWithCommitLog) CommitLog(ref string) ([]git.CommitInfo, error) {
...
func TestNewModel_CommitLogResolution(t *testing.T) {
	t.Run("explicit CommitLog wins over renderer capability", func(t *testing.T) {
		explicit := &fakeCommitLog{fn: func(string) ([]git.CommitInfo, error) {
			return []git.CommitInfo{{Hash: "explicit"}}, nil
		}}
		viaRenderer := &fakeCommitLog{fn: func(string) ([]git.CommitInfo, error) {
			return []git.CommitInfo{{Hash: "renderer"}}, nil
		}}
		m := testNewModel(t, rendererWithCommitLog{DiffSourceMock: plainRenderer(), commit: viaRenderer},
			annot.NewStore(), noopHighlighter(), ModelConfig{
				CommitLog:  explicit,
				Applicable: Applicable{CommitLog: true},
			})
	...
	t.Run("renderer capability is used when explicit is nil", func(t *testing.T) {
		viaRenderer := &fakeCommitLog{fn: func(string) ([]git.CommitInfo, error) {
			return []git.CommitInfo{{Hash: "from-renderer"}}, nil
		}}
		m := testNewModel(t, rendererWithCommitLog{DiffSourceMock: plainRenderer(), commit: viaRenderer},
			annot.NewStore(), noopHighlighter(), ModelConfig{Applicable: Applicable{CommitLog: true}})
	...
	t.Run("typed-nil explicit collapses to renderer fallback", func(t *testing.T) {
		...
		viaRenderer := &fakeCommitLog{fn: func(string) ([]git.CommitInfo, error) {
			return []git.CommitInfo{{Hash: "fallback"}}, nil
		}}
		m := testNewModel(t, rendererWithCommitLog{DiffSourceMock: plainRenderer(), commit: viaRenderer},
			annot.NewStore(), noopHighlighter(), ModelConfig{
				CommitLog:  typedNil,
	...
	t.Run("nil source when renderer lacks capability", func(t *testing.T) {
		m := testNewModel(t, plainRenderer(), annot.NewStore(), noopHighlighter(),
			ModelConfig{Applicable: Applicable{CommitLog: true}})
	...
	t.Run("applicable mirrors ModelConfig and source presence", func(t *testing.T) {
		...
		t.Run("not applicable -> false", func(t *testing.T) {
			m := testNewModel(t, rendererWithCommitLog{DiffSourceMock: plainRenderer(), commit: viaRenderer},
				annot.NewStore(), noopHighlighter(), ModelConfig{Applicable: Applicable{CommitLog: false}})
			assert.False(t, m.commits.applicable)
		})
	})
}
...
func TestModel_NewModel_ApplicableReload(t *testing.T) {
	plainRend := &mocks.DiffSourceMock{
		ChangedFilesFunc: func(string, bool) ([]git.FileEntry, error) { return nil, nil },
		FileDiffFunc:     func(git.FileDiffRequest) ([]git.DiffLine, error) { return nil, nil },
	}
	t.Run("true when Applicable.Reload is set", func(t *testing.T) {
		m := testNewModel(t, plainRend, annot.NewStore(), noopHighlighter(),
			ModelConfig{Applicable: Applicable{Reload: true}})
		assert.True(t, m.reload.applicable)
	})
	t.Run("false when Applicable.Reload is clear", func(t *testing.T) {
		m := testNewModel(t, plainRend, annot.NewStore(), noopHighlighter(),
			ModelConfig{Applicable: Applicable{Reload: false}})
...
func TestHandleChordSecond_LayoutFallback(t *testing.T) {
	...
	// Cyrillic che sits on the same physical key as Latin 'x' on a QWERTY layout.
...
func TestHandleOverlayOpen_InfoClearsChord(t *testing.T) {
	...
	m.commits.list = []git.CommitInfo{{Hash: "abc"}}
	m.keys.chordPending = "ctrl+w"
	m.keys.hint = "Pending: ctrl+w, esc to cancel"
...
func TestModel_AnnotatingNoOpMessageDoesNotRerenderDiff(t *testing.T) {
	// every message forwarded to the annotation input must not
	// force SetContent(renderDiff()), which is O(diff lines). the cursor blink alone fired
	// it twice a second on an idle session.
	lines := []git.DiffLine{
		{NewNum: 1, Content: "original one", ChangeType: git.ChangeContext},
		{NewNum: 2, Content: "original two", ChangeType: git.ChangeContext},
	}
	m := testModel([]string{"a.go"}, map[string][]git.DiffLine{"a.go": lines})
...
func TestNewModel_PageOverlap(t *testing.T) {
	...
	newModel := func(overlap int) Model {
		return testNewModel(t, renderer, annot.NewStore(), noopHighlighter(),
