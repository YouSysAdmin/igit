...
// the call, e.g. m.layout.focus.
func mouseTestModel(t *testing.T, files []string, diffs map[string][]git.DiffLine) Model {
	t.Helper()
...
func TestModel_HandleMouse_WheelInDiff(t *testing.T) {
	lines := make([]git.DiffLine, 60)
	for i := range lines {
		lines[i] = git.DiffLine{NewNum: i + 1, Content: "line", ChangeType: git.ChangeContext}
	}
	m := mouseTestModel(t, []string{"a.go"}, map[string][]git.DiffLine{"a.go": lines})
	m.file.lines = lines
...
func TestModel_HandleMouse_WheelInWrapMode_CursorAboveViewport(t *testing.T) {
	...
	lines := make([]git.DiffLine, 40)
	lines[0] = git.DiffLine{NewNum: 1, Content: strings.Repeat("X", 90), ChangeType: git.ChangeContext}
	for i := 1; i < len(lines); i++ {
		lines[i] = git.DiffLine{NewNum: i + 1, Content: "short", ChangeType: git.ChangeContext}
	}
	m := mouseTestModel(t, []string{"a.go"}, map[string][]git.DiffLine{"a.go": lines})
	m.file.lines = lines
	m.modes.wrap = true
...
func TestModel_HandleMouse_WheelInWrapMode_StraddlePinsToNextLine(t *testing.T) {
	...
	//
	// mouseTestModel layout: width=120, treeWidth=36, wrapWidth about 77 chars/row.
	...
	lines := make([]git.DiffLine, 40)
	lines[0] = git.DiffLine{NewNum: 1, Content: strings.Repeat("X", 350), ChangeType: git.ChangeContext}
	for i := 1; i < len(lines); i++ {
		lines[i] = git.DiffLine{NewNum: i + 1, Content: "short", ChangeType: git.ChangeContext}
	}
	m := mouseTestModel(t, []string{"a.go"}, map[string][]git.DiffLine{"a.go": lines})
...
func TestModel_HandleMouse_WheelInTreeMovesTreeCursor(t *testing.T) {
	...
	// - not NotEqual - so a multi-step move (PageDown(3)
	// behavior) would fail this test.
	files := make([]string, 30)
	diffs := make(map[string][]git.DiffLine, 30)
	for i := range files {
		p := string(rune('a'+(i/10))) + string(rune('a'+(i%10))) + ".go"
		files[i] = p
		diffs[p] = []git.DiffLine{{NewNum: 1, Content: p, ChangeType: git.ChangeContext}}
	}
	...
	t.Run("tree wheel does not touch diff wheelState", func(t *testing.T) {
		...
		// diff-pane debounce state. a change that accidentally bumped
...
func TestModel_HandleMouse_WheelNonPressActionIgnored(t *testing.T) {
	...
	m := mouseTestModel(t, []string{"a.go"}, map[string][]git.DiffLine{
		"a.go": {{NewNum: 1, Content: "hello", ChangeType: git.ChangeContext}},
	})
	m.nav.diffCursor = 0
...
func TestModel_HandleMouse_ShiftWheelInDiffUsesHalfPage(t *testing.T) {
	...
	// the keyboard half-page shortcut. tree wheel paths ignore the
	// magnitude entirely (single-step cursor nav) so they are not exercised
	// here.
	m := mouseTestModel(t, []string{"a.go"}, map[string][]git.DiffLine{
		"a.go": {{NewNum: 1, Content: "x", ChangeType: git.ChangeContext}},
	})
...
func TestModel_HandleMouse_LeftClickInDiff(t *testing.T) {
	...
	assert.False(t, model.nav.onAnnotationRow, "plain diff click must not set onAnnotationRow")
}
...
func TestModel_HandleMouse_LeftClickInDiffWithScrolledViewport(t *testing.T) {
	...
	assert.False(t, model.nav.onAnnotationRow)
}
...
func TestModel_HandleMouse_LeftClickSetsCursorOnAnnotation(t *testing.T) {
	...
	assert.True(t, model.nav.onAnnotationRow, "click on annotation sub-row must set onAnnotationRow")
}

func TestModel_HandleMouse_LeftClickOnDeleteOnlyPlaceholderDoesNotLandOnAnnotation(t *testing.T) {
	// in collapsed mode, a delete-only hunk renders a single "N lines deleted" placeholder
	...
	// onAnnotationRow, mirroring keyboard navigation guarded by
	...
	assert.False(t, model.nav.onAnnotationRow,
		"click on placeholder must not set onAnnotationRow — annotation is not rendered")
}
...
func TestModel_HandleMouse_WheelScrollsInfoOverlay(t *testing.T) {
	...
	commits := make([]git.CommitInfo, 0, 20)
	for range 20 {
		commits = append(commits, git.CommitInfo{Hash: "abc", Author: "a", Subject: "subject", Body: "body"})
	}
...
func TestModel_HandleMouse_WheelScrollsAnnotListOverlay(t *testing.T) {
	...
	items := []overlay.AnnotationItem{
		{File: "a.go", Line: 1, ChangeType: "+", Comment: "one"},
		{File: "a.go", Line: 2, ChangeType: "+", Comment: "two"},
		{File: "a.go", Line: 3, ChangeType: "+", Comment: "three"},
	}
...
func TestModel_HandleMouse_ClickConfirmsThemeSelect(t *testing.T) {
	...
	// to the confirmed entry ("dracula", not the initial "igit").
...
func TestModel_HandleMouse_ClickJumpsAnnotList(t *testing.T) {
	m := mouseTestModel(t, []string{"a.go", "b.go"}, map[string][]git.DiffLine{
		"a.go": {{NewNum: 1, Content: "a", ChangeType: git.ChangeContext}},
		"b.go": {{NewNum: 1, Content: "b", ChangeType: git.ChangeContext}},
	})
	mgr, ok := m.overlay.(*overlay.Manager)
	...
	items := []overlay.AnnotationItem{
		{File: "a.go", Line: 1, ChangeType: "+", Comment: "filler-0"},
		{File: "a.go", Line: 2, ChangeType: "+", Comment: "filler-1"},
		{File: "b.go", Line: 1, ChangeType: "+", Comment: "target"},
		{File: "a.go", Line: 3, ChangeType: "+", Comment: "filler-3"},
		{File: "a.go", Line: 4, ChangeType: "+", Comment: "filler-4"},
	}
	mgr.OpenAnnotList(overlay.AnnotListSpec{Items: items})
	ctx := overlay.RenderCtx{Width: m.layout.width, Height: m.layout.height, Resolver: m.resolver}
	_ = mgr.Compose(makeOverlayBase(m.layout.width, m.layout.height), ctx)
...
func TestModel_HandleMouse_StdinModeTreeHiddenClickLandsOnDiff(t *testing.T) {
	// a single-file review sets treeHidden = true with singleFile = true
	lines := []git.DiffLine{
		{NewNum: 1, Content: "line1", ChangeType: git.ChangeContext},
		{NewNum: 2, Content: "line2", ChangeType: git.ChangeContext},
		{NewNum: 3, Content: "line3", ChangeType: git.ChangeContext},
	}
	m := mouseTestModel(t, []string{"stdin"}, map[string][]git.DiffLine{"stdin": lines})
	m.file.lines = lines
	m.layout.treeHidden = true
	m.file.singleFile = true
	m.layout.treeWidth = 0
...
func TestModel_HandleMouse_WheelDebounceMsg_RendersOnMatchingGen(t *testing.T) {
	...
	// render (asserting against viewport.View() catches the case where
	...
	lines := make([]git.DiffLine, 60)
	for i := range lines {
		lines[i] = git.DiffLine{NewNum: i + 1, Content: fmt.Sprintf("line %d", i), ChangeType: git.ChangeContext}
	}
	m := mouseTestModel(t, []string{"a.go"}, map[string][]git.DiffLine{"a.go": lines})
	m.file.lines = lines
...
func TestModel_HandleMouse_WheelBurst_OppositeDirectionProcessesImmediately(t *testing.T) {
	// a wheel-up event arriving during a wheel-down
...
func TestModel_HandleBlameLoaded_FlushesPendingWheelBeforeSync(t *testing.T) {
	...
	result, _ = model.Update(blameLoadedMsg{
		...
		data: map[int]git.BlameLine{1: {Author: "alice"}},
	})
	model = result.(Model)
