...
func filePickerSpec() FilePickerSpec {
	return FilePickerSpec{
		Paths:      []string{"README.md", "internal/tui/model.go", "internal/tui/view.go", "docs/CONFIGURATION.md"},
		ActivePath: "internal/tui/model.go",
	}
}
...
func TestFilePickerOpenHighlightsCurrentFileAndCopiesInput(t *testing.T) {
	...
	assert.Equal(t, "internal/tui/model.go", mgr.filePick.entries[1])
}
...
func TestFilePickerFilterFullPathCaseInsensitive(t *testing.T) {
	...
	assert.Equal(t, []string{"internal/tui/view.go"}, mgr.filePick.entries)
	...
	assert.Contains(t, rendered, "internal/tui/view.go")
}
...
func TestFilePickerEnterSelectsAndCloses(t *testing.T) {
	...
	assert.Equal(t, "internal/tui/view.go", out.FileChoice.Path)
...
func TestFilePickerRenderSelectedPathAndWidth(t *testing.T) {
	...
	assert.Contains(t, rendered, "> internal/tui/model.go")
