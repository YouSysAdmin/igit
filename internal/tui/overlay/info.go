...
const (
	...
	infoShortHashLen     = 12
	infoErrMaxLen        = 1000 // upper bound on rendered git-error text, raw stderr can be megabytes
)
...
// buildContent assembles the wrapped, padded content for the popup. Two
// optional sections render top-to-bottom: session info and the commit log. Each section that has nothing to show is skipped
...
func (c *infoOverlay) buildContent(innerWidth int, resolver Resolver) []string {
	...
	if len(out) == 0 {
		...
		// filters, no commits applicable). A single padded
...
// buildDetailRows renders the "details" section: the rows the caller passes
// in, typically the active filters, the compact options and the working
// directory. Returns nil when no row has content, which hides the section
...
func (c *infoOverlay) buildDetailRows(innerWidth int, resolver Resolver) []string {
	...
	for _, row := range c.spec.Rows {
		...
		// row values can carry upstream-supplied text (raw git stderr in the
...
func (c *infoOverlay) buildCommitsSection(innerWidth int, resolver Resolver) []string {
	...
	case c.spec.CommitsErr != nil:
		// git errors carry raw stderr, which may contain newlines/tabs and be
		// megabytes long. Fields collapses all
...
func (c *infoOverlay) renderCommit(commit git.CommitInfo, innerWidth int, resolver Resolver) []string {
	var out []string
...
// popups. exported so the tui package can reuse the same constant for diff-pane wheel
...
// not otherwise bounded (raw git stderr can be megabytes).
...
// git.SanitizeCommitText so all untrusted text in the overlay routes through
// one strip path with identical semantics (full ANSI CSI sequences, ESC,
// C0/DEL/C1 controls, git framing delimiters, invalid UTF-8). After stripping,
