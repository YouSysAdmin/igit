...
func makeCommit(hash, author, subject, body string, date time.Time) git.CommitInfo {
...
func TestInfoOverlay_DefaultTitleWhenNoHeaderText(t *testing.T) {
	...
	mgr.OpenInfo(InfoSpec{
		...
		Commits: []git.CommitInfo{{Hash: "abc"}},
	})
	out := mgr.info.render(infoRenderCtx(), mgr)
...
func TestInfoOverlay_FooterGracefullyDegradesOnNarrowPopup(t *testing.T) {
	// narrow terminals shouldn't crash or render a corrupted bottom
...
func TestInfoOverlay_DetailRowMutedSuffixRendered(t *testing.T) {
	...
	mgr.OpenInfo(InfoSpec{
		HeaderText: "info",
		Rows: []InfoRow{
			{Label: "project", Value: "igit", MutedSuffix: "/home/sasha/Developer/igit"},
		},
	})
	out := mgr.info.render(RenderCtx{Width: 100, Height: 12, Resolver: resolver}, mgr)

	assert.Contains(t, out, "igit", "primary value rendered")
	assert.Contains(t, out, "/home/sasha/Developer/igit", "muted suffix appended after value")
	...
	valueIdx := strings.Index(out, "igit")
...
func TestInfoOverlay_RenderCommitsHiddenWhenNotApplicable(t *testing.T) {
	// in the merged overlay the commits section is HIDDEN entirely
...
func TestInfoOverlay_RenderCommitsLoadingState(t *testing.T) {
	// in the merged overlay, opening the popup before commits
...
func TestInfoOverlay_RenderSingleCommit(t *testing.T) {
	...
	mgr := NewManager()
	mgr.OpenInfo(InfoSpec{CommitsApplicable: true, CommitsLoaded: true, Commits: []git.CommitInfo{commit}})
	out := mgr.info.render(infoRenderCtx(), mgr)
...
func TestInfoOverlay_RenderTruncatedTitle(t *testing.T) {
	...
	mgr := NewManager()
	mgr.OpenInfo(InfoSpec{CommitsApplicable: true, CommitsLoaded: true, Truncated: true, Commits: []git.CommitInfo{commit}})
	out := mgr.info.render(infoRenderCtx(), mgr)
	assert.Contains(t, out, "truncated")
}
...
func TestInfoOverlay_ScrollJK(t *testing.T) {
	...
	commits := make([]git.CommitInfo, 0, 30)
	for range 30 {
...
func TestInfoOverlay_ManyCommitsRender(t *testing.T) {
	commits := make([]git.CommitInfo, 0, 10)
	for range 10 {
...
func TestInfoOverlay_AnsiInjectionRendersLiteral(t *testing.T) {
	// the parser strips \x1b before the overlay sees the text, so any brackets
	// that reach it are literal text, not escape sequences.
...
func TestSanitizeInfoText_stripsFullCSISequences(t *testing.T) {
	...
	// visible. After routing through git.SanitizeCommitText, the entire CSI
	// sequence is removed.
	c := &infoOverlay{}
