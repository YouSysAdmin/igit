// Package overlay owns all layered popup UI for igit - help, annotation list,
...
// from HandleKey. The overlay package has no dependency on tui.Model, annotation store,
...
const (
	...
	KindConfirm          // yes/no confirmation
	KindMenu             // small action menu
	KindError            // scrollable error/message popup
	KindPrompt           // single-line text prompt
)
...
const (
	...
	OutcomeConfirmed                           // user confirmed (id in Outcome.ConfirmID)
	OutcomeMenuChosen                          // user picked a menu item (id in Outcome.MenuChoice)
	OutcomePromptSubmitted                     // user submitted the prompt (Outcome.PromptID/PromptValue)
	OutcomePromptEditor                        // user asked to continue the prompt in an editor (Outcome.PromptID/PromptValue)
)
...
type Outcome struct {
	...
	ConfirmID        string // ConfirmSpec.ID for OutcomeConfirmed
	MenuChoice       string // MenuItem.ID for OutcomeMenuChosen
	PromptID         string // PromptSpec.ID for the prompt outcomes
	PromptValue      string // prompt text for the prompt outcomes
}
...
// InfoSpec describes the info popup: session rows and, for a ref range, the
// commit log. CommitsApplicable false hides the commit section entirely
// instead of showing an empty one. Truncated marks a list capped at
// git.MaxCommits, CommitsErr replaces the empty-list message with the fetch
// error.
...
// OpenConfirm shows a yes/no confirmation popup.
func (m *Manager) OpenConfirm(spec ConfirmSpec) {
	m.confirm.open(spec)
	m.kind = KindConfirm
}

// OpenMenu shows a small action menu.
func (m *Manager) OpenMenu(spec MenuSpec) {
	m.menu.open(spec)
	m.kind = KindMenu
}

// OpenPrompt shows a single-line text prompt.
func (m *Manager) OpenPrompt(spec PromptSpec) {
	m.prompt.open(spec)
	m.kind = KindPrompt
}

// PromptValue returns the text currently typed in the prompt ("" when closed).
func (m *Manager) PromptValue() string {
...
// OpenError shows a scrollable message popup.
func (m *Manager) OpenError(spec ErrorSpec) {
	m.errPop.open(spec)
	m.kind = KindError
}
...
func (m *Manager) HandleKey(msg tea.KeyMsg, action keymap.Action) Outcome {
	...
	case KindConfirm:
		out = m.confirm.handleKey(msg, action)
	case KindMenu:
		out = m.menu.handleKey(msg, action)
	case KindError:
		out = m.errPop.handleKey(msg, action)
	case KindPrompt:
		out = m.prompt.handleKey(msg, action)
	default:
		return Outcome{}
	}
...
// closeOnOutcome closes the overlay for every outcome that ends its session.
func (m *Manager) closeOnOutcome(out Outcome) {
	...
	case OutcomeClosed, OutcomeAnnotationChosen, OutcomeThemeConfirmed, OutcomeThemeCanceled, OutcomeFileChosen,
		OutcomeConfirmed, OutcomeMenuChosen, OutcomePromptSubmitted, OutcomePromptEditor:
		m.Close()
	case OutcomeNone, OutcomeThemePreview: // no state change
	}
}

// HandleMouse routes a mouse event to the active overlay: the wheel scrolls
// it, a left click inside the popup hit-tests an item row and can select it.
// Click coordinates are translated to popup-local ones first. Clicks outside
// the popup and other buttons are swallowed, and an outcome that implies
// dismissal closes the overlay, as in HandleKey.
func (m *Manager) HandleMouse(msg tea.MouseMsg) Outcome {
	...
	case KindConfirm:
		out = m.confirm.handleMouse(msg)
	case KindMenu:
		out = m.menu.handleMouse(msg)
	case KindError:
		out = m.errPop.handleMouse(msg)
	case KindPrompt:
		out = m.prompt.handleMouse(msg)
...
func (m *Manager) Compose(base string, ctx RenderCtx) string {
	...
	case KindConfirm:
		fg = m.confirm.render(ctx, m)
	case KindMenu:
		fg = m.menu.render(ctx, m)
	case KindError:
		fg = m.errPop.render(ctx, m)
	case KindPrompt:
		fg = m.prompt.render(ctx, m)
	}
