...
func TestApplyScrollbar_NeverModifiesCorners(t *testing.T) {
	...
	// top border row 0 contains the corner runes, never replaced
	assert.Contains(t, lines[0], "┌")
	assert.Contains(t, lines[0], "┐")
	assert.NotContains(t, lines[0], scrollbarThumbRune)

	// bottom border row contains the corner runes, never replaced
	last := len(lines) - 1
...
// lock in the bold SGR contract so a future edit that drops the
...
// cover the idx<0 branch - a viewport row that does not contain the
...
func TestApplyScrollbar_RowWithoutTrackRune(t *testing.T) {
	...
	// row 2 (first viewport row) has no border rune - must be skipped without panic
	rendered := strings.Join([]string{
		"┌────────┐",
		"│header  │",
		"plain row no border",
...
// a viewport row whose content contains a literal bar rune (reviewing a
// markdown table or box-drawing source) must not have that rune touched,
// only the rightmost (border) rune is replaced. locks in the assumption
...
func TestApplyScrollbar_BodyContainingTrackRune(t *testing.T) {
	...
	rendered := strings.Join([]string{
		"┌──────────────┐",
		"│header        │",
		"│ a │ b │ c    │", // body with multiple bar runes
		"│ a │ b │ c    │",
		"│ a │ b │ c    │",
		"└──────────────┘",
	}, "\n")
...
// integration test that runs applyScrollbar against the real lipgloss
...
// smallest non-trivial viewport. with vh=1 the thumb has nowhere to
...
// defensive bail when the rendered pane has more rows
