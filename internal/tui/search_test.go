...
func TestModel_HighlightSearchMatches(t *testing.T) {
	...
	t.Run("plain text single match", func(t *testing.T) {
		m.search.term = "hello"
		result := m.highlightSearchMatches("say hello world", git.ChangeContext)
	...
	t.Run("multiple matches", func(t *testing.T) {
		m.search.term = "ab"
		result := m.highlightSearchMatches("ab cd ab", git.ChangeContext)
		assert.Equal(t, 2, strings.Count(result, "\033[48;2;215;215;0m"), "should highlight both occurrences")
	})

	t.Run("no match", func(t *testing.T) {
		m.search.term = "xyz"
		result := m.highlightSearchMatches("hello world", git.ChangeContext)
	...
	t.Run("with ansi codes", func(t *testing.T) {
		m.search.term = "world"
		result := m.highlightSearchMatches("\033[32mhello world\033[0m", git.ChangeContext)
	...
	t.Run("no-colors fallback", func(t *testing.T) {
		...
		result := noColorModel.highlightSearchMatches("say hello world", git.ChangeContext)
	...
	t.Run("add line restores add bg instead of terminal default", func(t *testing.T) {
		...
		result := am.highlightSearchMatches("say hello world", git.ChangeAdd)
	...
	t.Run("remove line restores remove bg instead of terminal default", func(t *testing.T) {
		...
		result := rm.highlightSearchMatches("say hello world", git.ChangeRemove)
		assert.Contains(t, result, "\033[48;2;215;215;0m", "should have search bg on")
		assert.NotContains(t, result, "\033[49m", "should not reset to terminal default")
	...
	t.Run("search inside word-diff span restores word-diff bg", func(t *testing.T) {
		...
		result := am.highlightSearchMatches(input, git.ChangeAdd)
...
func TestModel_SubmitSearchFindsMatches(t *testing.T) {
	lines := []git.DiffLine{
		{NewNum: 1, Content: "hello world", ChangeType: git.ChangeContext},
		{NewNum: 2, Content: "foo bar", ChangeType: git.ChangeAdd},
		{NewNum: 3, Content: "hello again", ChangeType: git.ChangeContext},
	}
	m := testModel([]string{"a.go"}, map[string][]git.DiffLine{"a.go": lines})
	result, _ := m.Update(tea.WindowSizeMsg{Width: 120, Height: 40})
	model := result.(Model)
...
func TestModel_SubmitSearchCaseInsensitive(t *testing.T) {
	lines := []git.DiffLine{
		{NewNum: 1, Content: "Hello World", ChangeType: git.ChangeContext},
		{NewNum: 2, Content: "HELLO again", ChangeType: git.ChangeAdd},
	}
	m := testModel([]string{"a.go"}, map[string][]git.DiffLine{"a.go": lines})
	result, _ := m.Update(tea.WindowSizeMsg{Width: 120, Height: 40})
	model := result.(Model)
...
func TestModel_SubmitSearchJumpsForwardFromCursor(t *testing.T) {
	lines := []git.DiffLine{
		{NewNum: 1, Content: "match here", ChangeType: git.ChangeContext},
		{NewNum: 2, Content: "foo bar", ChangeType: git.ChangeAdd},
		{NewNum: 3, Content: "match again", ChangeType: git.ChangeContext},
	}
	m := testModel([]string{"a.go"}, map[string][]git.DiffLine{"a.go": lines})
	result, _ := m.Update(tea.WindowSizeMsg{Width: 120, Height: 40})
	model := result.(Model)
...
func TestModel_SubmitSearchNoMatches(t *testing.T) {
	lines := []git.DiffLine{
		{NewNum: 1, Content: "hello", ChangeType: git.ChangeContext},
	}
	m := testModel([]string{"a.go"}, map[string][]git.DiffLine{"a.go": lines})
	result, _ := m.Update(tea.WindowSizeMsg{Width: 120, Height: 40})
	model := result.(Model)
...
func TestModel_SearchInputForwardsCharacters(t *testing.T) {
	lines := []git.DiffLine{{NewNum: 1, Content: "hello", ChangeType: git.ChangeContext}}
	m := testModel([]string{"a.go"}, map[string][]git.DiffLine{"a.go": lines})
	result, _ := m.Update(tea.WindowSizeMsg{Width: 120, Height: 40})
	model := result.(Model)
	result, _ = model.Update(fileLoadedMsg{file: "a.go", lines: lines})
...
func TestModel_NextSearchMatch(t *testing.T) {
	lines := []git.DiffLine{
		...
		{NewNum: 2, Content: "no match", ChangeType: git.ChangeAdd},
		{NewNum: 3, Content: "match two", ChangeType: git.ChangeContext},
		{NewNum: 4, Content: "match three", ChangeType: git.ChangeAdd},
	}
	m := testModel([]string{"a.go"}, map[string][]git.DiffLine{"a.go": lines})
	result, _ := m.Update(tea.WindowSizeMsg{Width: 120, Height: 40})
	model := result.(Model)
...
func TestModel_SearchNavigationSkipsCollapsedHiddenLines(t *testing.T) {
	...
	lines := []git.DiffLine{
		{NewNum: 1, Content: "match ctx", ChangeType: git.ChangeContext},
		{OldNum: 2, Content: "match removed", ChangeType: git.ChangeRemove},
		{NewNum: 2, Content: "match added", ChangeType: git.ChangeAdd},
		{NewNum: 3, Content: "match end", ChangeType: git.ChangeContext},
	}
	m := testModel([]string{"a.go"}, map[string][]git.DiffLine{"a.go": lines})
	result, _ := m.Update(tea.WindowSizeMsg{Width: 120, Height: 40})
	model := result.(Model)
...
func TestModel_NKeyFallsThroughToNextFileWhenNoSearch(t *testing.T) {
	lines := []git.DiffLine{{NewNum: 1, Content: "line1", ChangeType: git.ChangeContext}}
	m := testModel([]string{"a.go", "b.go"}, map[string][]git.DiffLine{
...
func TestModel_ShiftNDoesPrevMatchWhenSearchActive(t *testing.T) {
	lines := []git.DiffLine{
		{NewNum: 1, Content: "match one", ChangeType: git.ChangeContext},
		{NewNum: 2, Content: "match two", ChangeType: git.ChangeAdd},
	}
	m := testModel([]string{"a.go"}, map[string][]git.DiffLine{"a.go": lines})
	result, _ := m.Update(tea.WindowSizeMsg{Width: 120, Height: 40})
	model := result.(Model)
...
func TestModel_SearchHighlightInRenderDiff(t *testing.T) {
	lines := []git.DiffLine{
		...
		{NewNum: 2, Content: "func hello() {}", ChangeType: git.ChangeAdd},
		{NewNum: 3, Content: "func world() {}", ChangeType: git.ChangeAdd},
		{OldNum: 4, Content: "old line", ChangeType: git.ChangeRemove},
	}
	m := testModel([]string{"a.go"}, map[string][]git.DiffLine{"a.go": lines})
...
func TestModel_SearchHighlightWithWrap(t *testing.T) {
	...
	lines := []git.DiffLine{
		{NewNum: 1, Content: longContent, ChangeType: git.ChangeAdd},
		{NewNum: 2, Content: "short line", ChangeType: git.ChangeAdd},
	}
	m := testModel([]string{"a.go"}, map[string][]git.DiffLine{"a.go": lines})
	m.tree = testNewFileTree([]string{"a.go"})
	m.file.name = "a.go"
	...
	// the long line should produce continuation rows with the wrap marker
...
func TestModel_SearchHighlightInCollapsedMode(t *testing.T) {
	lines := []git.DiffLine{
		...
		{NewNum: 2, Content: "added hello line", ChangeType: git.ChangeAdd},
		{NewNum: 3, Content: "added other line", ChangeType: git.ChangeAdd},
	}
	m := testModel([]string{"a.go"}, map[string][]git.DiffLine{"a.go": lines})
	m.tree = testNewFileTree([]string{"a.go"})
	m.file.name = "a.go"
...
func TestModel_StyleDiffContentSearchMatch(t *testing.T) {
	...
	t.Run("search match returns same text content", func(t *testing.T) {
		resultMatch := m.styleDiffContent(git.ChangeAdd, " + ", "content", false, true)
		resultNoMatch := m.styleDiffContent(git.ChangeAdd, " + ", "content", false, false)
	...
	t.Run("search match with highlight preserves content", func(t *testing.T) {
		result := m.styleDiffContent(git.ChangeAdd, " + ", "\033[32mgreen\033[0m", true, true)
...
func TestModel_StatusBarSearchMatchPosition(t *testing.T) {
	tests := []struct {
		...
		{name: "no matches", matches: nil, cursor: 0, wantAbsent: "0/0"},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.wantAbsent != "" {
				...
				assert.Empty(t, m.searchSegment(), "no matches leaves the search segment out")
			}
		})
	}
}
...
func TestModel_SearchSegment(t *testing.T) {
	...
	lines := []git.DiffLine{
		{NewNum: 1, Content: "ctx", ChangeType: git.ChangeContext},
		{OldNum: 2, Content: "removed match", ChangeType: git.ChangeRemove},
		{NewNum: 2, Content: "added", ChangeType: git.ChangeAdd},
		{NewNum: 3, Content: "ctx end", ChangeType: git.ChangeContext},
	}
...
func TestModel_StatusBarSearchPositionBetweenHunkAndIcons(t *testing.T) {
	...
	// [1/1] should appear after hunk and before the mode icons
...
func TestModel_ClearSearchOnFileLoad(t *testing.T) {
	...
	lines2 := []git.DiffLine{
		{NewNum: 1, Content: "other content", ChangeType: git.ChangeContext},
	}
	m := testModel([]string{"a.go", "b.go"}, map[string][]git.DiffLine{"a.go": lines1, "b.go": lines2})
	result, _ := m.Update(tea.WindowSizeMsg{Width: 120, Height: 40})
	model := result.(Model)
...
func TestModel_SubmitSearchPreservesLeadingWhitespace(t *testing.T) {
	...
	lines := []git.DiffLine{
		{NewNum: 1, Content: "  indented line", ChangeType: git.ChangeContext},
		{NewNum: 2, Content: "normal line", ChangeType: git.ChangeContext},
	}
	m := testModel([]string{"a.go"}, map[string][]git.DiffLine{"a.go": lines})
	result, _ := m.Update(tea.WindowSizeMsg{Width: 120, Height: 40})
	model := result.(Model)
...
func TestModel_DeletePlaceholderSearchHighlight(t *testing.T) {
	...
	lines := []git.DiffLine{
		{NewNum: 1, Content: "context", ChangeType: git.ChangeContext},
		{OldNum: 2, Content: "deleted match", ChangeType: git.ChangeRemove},
		{OldNum: 3, Content: "deleted other", ChangeType: git.ChangeRemove},
		{NewNum: 2, Content: "context end", ChangeType: git.ChangeContext},
	}
	m := testModel([]string{"a.go"}, map[string][]git.DiffLine{"a.go": lines})
	result, _ := m.Update(tea.WindowSizeMsg{Width: 120, Height: 40})
	model := result.(Model)
...
func newSearchHistoryModel(t *testing.T) Model {
	t.Helper()
	lines := []git.DiffLine{
		{NewNum: 1, Content: "alpha line", ChangeType: git.ChangeContext},
		{NewNum: 2, Content: "beta added", ChangeType: git.ChangeAdd},
		{NewNum: 3, Content: "gamma context", ChangeType: git.ChangeContext},
	}
	m := testModel([]string{"a.go"}, map[string][]git.DiffLine{"a.go": lines})
	result, _ := m.Update(tea.WindowSizeMsg{Width: 120, Height: 40})
	model := result.(Model)
