...
type FileTree struct {
	...
	fileStatuses map[string]git.ChangeStatus // file change status from git, empty for non-git
...
func NewFileTree(entries []git.FileEntry) *FileTree {
	paths := git.FileEntryPaths(entries)
	ft := &FileTree{
		allFiles:     paths,
		reviewed:     make(map[string]string),
		fileStatuses: make(map[string]git.ChangeStatus),
...
func (ft *FileTree) FileStatus(path string) git.ChangeStatus {
...
func (ft *FileTree) Rebuild(entries []git.FileEntry) {
	...
	newStatuses := make(map[string]git.ChangeStatus, len(entries))
...
func (ft *FileTree) Render(r FileTreeRender) string {
	...
	rc := renderCtx{annotatedFiles: r.Annotated, stageMarked: r.StageMarked, res: r.Resolver, rnd: r.MarkRenderer}
...
func (ft *FileTree) renderFileEntry(e treeEntry, idx, width int, rc renderCtx) string {
	...
	marker := "  "
	switch {
	case rc.annotatedFiles[e.path]:
		marker = rc.rnd.FileAnnotationMark()
	case rc.stageMarked[e.path] && isSelected:
		marker = " +"
	case rc.stageMarked[e.path]:
		marker = rc.rnd.FileStageMark()
	}
