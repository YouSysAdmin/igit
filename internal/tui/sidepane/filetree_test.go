...
// fileEntries is a test helper converting a slice of file paths to git.FileEntry.
func fileEntries(paths ...string) []git.FileEntry {
...
func TestFileTree_UnreviewedAdvanceFollowsDisplayOrder(t *testing.T) {
	// MarkRenderer order differs from the tree's directory-grouped display order:
	// b.go, a/c.go, a/d.go.
...
func TestFileTree_UnreviewedFilterEmptyState(t *testing.T) {
	...
	result := ft.Render(FileTreeRender{Width: 30, Height: 10, Resolver: res, MarkRenderer: rnd})
...
func TestFileTree_Render(t *testing.T) {
	...
	result := ft.Render(FileTreeRender{Width: 30, Height: 100, Annotated: map[string]bool{"internal/handler.go": true}, Resolver: res, MarkRenderer: rnd})
...
func TestFileTree_RenderEmpty(t *testing.T) {
	...
	res := style.NewResolver(style.Colors{Accent: "#5f87ff", Border: "#585858", Normal: "#d0d0d0", Muted: "#6c6c6c", SelectedFg: "#ffffaf", SelectedBg: "#303030", Annotation: "#ffd700", CursorBg: "#3a3a3a", AddFg: "#87d787", AddBg: "#022800", RemoveFg: "#ff8787", RemoveBg: "#3D0100"})
	rnd := style.NewRenderer(res)
	result := ft.Render(FileTreeRender{Width: 30, Height: 100, Resolver: res, MarkRenderer: rnd})
...
func TestFileTree_RenderIndentation(t *testing.T) {
	...
	result := ft.Render(FileTreeRender{Width: 40, Height: 100, Resolver: res, MarkRenderer: rnd})
...
func TestFileTree_RenderViewport(t *testing.T) {
	...
	result := ft.Render(FileTreeRender{Width: 40, Height: 3, Resolver: res, MarkRenderer: rnd})
	lines := strings.Split(result, "\n")
...
func TestFileTree_RenderTruncatesLongDirNames(t *testing.T) {
	ft := NewFileTree(fileEntries("tools/skills/igit/references/config.md"))
	...
	assert.NotContains(t, result, "tools/skills/igit/references/", "full dir name should not appear")
}
...
func TestFileTree_RenderViewportCursorAlwaysVisible(t *testing.T) {
	...
	result := ft.Render(FileTreeRender{Width: 40, Height: 5, Resolver: res, MarkRenderer: rnd})
	assert.Contains(t, result, "h.go", "cursor file must be visible after page down")

	ft.Move(MotionPageUp, 100)
	result = ft.Render(FileTreeRender{Width: 40, Height: 5, Resolver: res, MarkRenderer: rnd})
	selected := ft.SelectedFile()
	assert.Contains(t, result, path.Base(selected),
...
func TestFileTree_RenderReviewedCheckmark(t *testing.T) {
	...
	result := ft.Render(FileTreeRender{Width: 40, Height: 10, Resolver: res, MarkRenderer: rnd})
	...
	result = ft.Render(FileTreeRender{Width: 40, Height: 10, Annotated: annotated, Resolver: res, MarkRenderer: rnd})
	assert.Contains(t, result, "✓")
	assert.Contains(t, result, "*")
}

func TestFileTree_RenderFileEntryRestoresNormalForegroundAfterColoredPrefix(t *testing.T) {
	ft := NewFileTree([]git.FileEntry{{Path: "a.go", Status: git.FileAdded}})
...
func TestFileTreeRebuild(t *testing.T) {
	...
	t.Run("fileStatuses refreshed from new entries", func(t *testing.T) {
		ft.Rebuild([]git.FileEntry{{Path: "m.go", Status: git.FileModified}, {Path: "n.go", Status: git.FileAdded}})
		assert.Equal(t, git.FileModified, ft.FileStatus("m.go"))
		assert.Equal(t, git.FileAdded, ft.FileStatus("n.go"))
		assert.Equal(t, git.ChangeStatus(""), ft.FileStatus("x.go"), "old statuses should be cleared")
	})
...
func TestFileTree_OldPath(t *testing.T) {
	entries := []git.FileEntry{
		{Path: "new.go", OldPath: "old.go", Status: git.FileRenamed},
		{Path: "mod.go", Status: git.FileModified},
	}
	...
	t.Run("survives rebuild with refreshed origins", func(t *testing.T) {
		ft.Rebuild([]git.FileEntry{
			{Path: "dst.go", OldPath: "src.go", Status: git.FileRenamed},
		})
...
func TestFileTree_ScrollState(t *testing.T) {
	...
	t.Run("offset reflects post-Render state via the Render path", func(t *testing.T) {
		...
		rnd := style.NewRenderer(res)
		_ = ft.Render(FileTreeRender{Width: 30, Height: 10, Resolver: res, MarkRenderer: rnd})
