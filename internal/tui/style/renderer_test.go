...
func TestRenderer_FileStatusMark_plain(t *testing.T) {
	rnd := NewRenderer(PlainResolver())
	got := rnd.FileStatusMark(git.FileAdded)
	assert.Equal(t, "A ", got, "plain mode has no ANSI codes, just status + space")
}

// TestRenderer_FileStatusMark_emptyNormalFallsBackToResetFg pins the color-leak
// guard: with Colors.Normal unset the status color must still end in ResetFg
// ("\033[39m"), so it cannot bleed into text rendered after the mark on the
// same line and bg stays untouched.
func TestRenderer_FileStatusMark_emptyNormalFallsBackToResetFg(t *testing.T) {
	...
	for _, status := range []git.ChangeStatus{git.FileAdded, git.FileDeleted, git.FileModified} {
...
func TestRenderer_FileReviewedMark(t *testing.T) {
	...
	// with Colors.Normal unset the AddFg color must still be terminated
	// by ResetFg so it cannot bleed into subsequent text.
