...
// and dispatches lookups by key or by git.ChangeType.
...
func (r Resolver) LineBg(change git.ChangeType) Color {
	switch change {
	case git.ChangeAdd:
		return Color(ansiColor(r.colors.AddBg, 48))
...
func (r Resolver) LineFg(change git.ChangeType) Color {
	switch change {
	case git.ChangeAdd:
		return Color(ansiColor(r.colors.AddFg, 38))
	case git.ChangeRemove:
...
func (r Resolver) LineStyle(change git.ChangeType, highlighted bool) lipgloss.Style {
	switch {
	case change == git.ChangeAdd && highlighted:
		return r.Style(StyleKeyLineAddHighlight)
	case change == git.ChangeAdd:
		return r.Style(StyleKeyLineAdd)
	case change == git.ChangeRemove && highlighted:
		return r.Style(StyleKeyLineRemoveHighlight)
	case change == git.ChangeRemove:
...
func (r Resolver) WordDiffBg(change git.ChangeType) Color {
	switch change {
	case git.ChangeAdd:
		return Color(ansiColor(r.colors.WordAddBg, 48))
	case git.ChangeRemove:
...
func (r Resolver) IndicatorBg(change git.ChangeType) Color {
	bg := r.LineBg(change)
	if bg != "" {
		return bg
	}
...
func buildStyles(c Colors) map[StyleKey]lipgloss.Style {
	...
	// annotation input styles (from annotate.go)
	...
	// annotation list popup border (from annotlist.go)
	...
	// help overlay box (from handlers.go)
	...
	// theme selector box (from themeselect.go)
