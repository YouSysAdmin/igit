...
func TestResolver_AnnotationInputStyles(t *testing.T) {
	// input text resolved to Normal, making it byte-identical to a context line
	r := NewResolver(fullColorsForTesting)
...
func TestResolver_LineBg(t *testing.T) {
	...
	tests := []struct {
		name   string
		change git.ChangeType
		want   bool // true if non-empty expected
	}{
		{"add", git.ChangeAdd, true},
		{"remove", git.ChangeRemove, true},
		{"context", git.ChangeContext, false},
		{"divider", git.ChangeDivider, false},
	}
...
func TestResolver_LineStyle(t *testing.T) {
	...
	tests := []struct {
		...
		{"add plain", git.ChangeAdd, false, StyleKeyLineAdd},
		{"add highlighted", git.ChangeAdd, true, StyleKeyLineAddHighlight},
		{"remove plain", git.ChangeRemove, false, StyleKeyLineRemove},
		{"remove highlighted", git.ChangeRemove, true, StyleKeyLineRemoveHighlight},
		{"context plain", git.ChangeContext, false, StyleKeyLineContext},
		{"context highlighted", git.ChangeContext, true, StyleKeyLineContextHighlight},
	}
...
func TestResolver_IndicatorBg(t *testing.T) {
	...
	t.Run("add line uses add bg", func(t *testing.T) {
		got := r.IndicatorBg(git.ChangeAdd)
		assert.Equal(t, r.LineBg(git.ChangeAdd), got)
	})

	t.Run("remove line uses remove bg", func(t *testing.T) {
		got := r.IndicatorBg(git.ChangeRemove)
		assert.Equal(t, r.LineBg(git.ChangeRemove), got)
	})

	t.Run("context falls back to diff pane bg", func(t *testing.T) {
		got := r.IndicatorBg(git.ChangeContext)
	...
	t.Run("divider falls back to diff pane bg", func(t *testing.T) {
		got := r.IndicatorBg(git.ChangeDivider)
		assert.Equal(t, r.Color(ColorKeyDiffPaneBg), got)
	})
}
...
func TestPlainResolver_LineBg(t *testing.T) {
	...
	assert.Empty(t, string(r.LineBg(git.ChangeAdd)))
	assert.Empty(t, string(r.LineBg(git.ChangeRemove)))
	assert.Empty(t, string(r.LineBg(git.ChangeContext)))
}

func TestPlainResolver_WordDiffBg(t *testing.T) {
	r := PlainResolver()
	assert.Empty(t, string(r.WordDiffBg(git.ChangeAdd)))
	assert.Empty(t, string(r.WordDiffBg(git.ChangeRemove)))
}
