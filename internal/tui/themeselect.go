...
// adoptStyle takes over the styling and theme name of another model (the
// commit mode's diff pane after a theme was chosen there). Both models share
// the highlighter, so only the resolver-derived state and the cached rendering
// need refreshing here.
func (m *Model) adoptStyle(from Model) {
	m.resolver, m.renderer, m.sgr = from.resolver, from.renderer, from.sgr
	m.activeThemeName = from.activeThemeName
	m.styleGen++
	m.invalidateRenderCaches()
	m.refreshDiff()
}
