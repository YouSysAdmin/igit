...
// newTestThemeCatalog returns a catalog pre-populated with igit, dracula, and nord themes.
func newTestThemeCatalog() *testThemeCatalog {
	entries := []ThemeEntry{
		{Name: "igit", Local: false, AccentColor: "#61afef"},
...
func TestApplyTheme_invalidChromaStyleKeepsPreviousHighlightingStyle(t *testing.T) {
	...
	highlighter := &mocks.SyntaxHighlighterMock{
		HighlightLinesFunc: func(string, []git.DiffLine) []string {
			highlightCalls++
	...
	m.file.name = "main.go"
	m.file.lines = []git.DiffLine{{ChangeType: git.ChangeContext, Content: "package main"}}
